...
func modelsSetCommand() *ucli.Command {
	return &ucli.Command{
		Name:      "set",
		Usage:     "Switch the active model (e.g. stella models set openai/gpt-4o)",
...
func modelsSearchCommand() *ucli.Command {
	return &ucli.Command{
		Name:      "search",
		Usage:     "Search models by name (e.g. stella models search gpt)",
		ArgsUsage: "<query>",
		Action: func(c *ucli.Context) error {
			query := strings.TrimSpace(c.Args().First())
			if query == "" {
				return fmt.Errorf("usage: stella models search <query>")
			}
...
func parseProviderModelArg(arg string) (string, string, error) {
	if arg == "" {
		return "", "", fmt.Errorf("usage: stella models set <provider/model>")
	}
