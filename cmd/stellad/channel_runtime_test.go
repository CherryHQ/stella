...
func TestApplyManagedChannelPluginsDoesNotStartRuntimeWhenNoChannels(t *testing.T) {
	host := &fakeManagedChannelRuntimeHost{
		configured: map[string]bool{
			"telegram": true,
			"feishu":   true,
	...
	if len(host.reconcileChannelCalls) != 0 {
		t.Fatalf("runtime startup calls = %v; want none", host.reconcileChannelCalls)
	}
}
