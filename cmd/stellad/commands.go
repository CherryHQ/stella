...
func modelMemoryToolAvailability(structuredGroupMemory bool) func(context.Context, agent.RunnerParams) bool {
	if structuredGroupMemory {
		// Structured group turns receive Group Facts in the system prompt and
		// public history through their per-Agent LCM. The unified memory tool is
		// private user recall, so it remains available only outside groups.
		return agent.NonGroupBuiltinToolAvailable
	}
	return nil
}
...
func setup(parent context.Context, cfg config.ServerConfig, baseURL string) (*setupResult, error) {
	...
	builtinTools = append([]agent.BuiltinTool{{
		...
		Available: modelMemoryToolAvailability(groupMemory.structured),
	}}, builtinTools...)
