...
- `mise run test:e2e` runs the functional Playwright project in `test/e2e/`; `mise run test:e2e:fast` excludes `@model` turns. Performance runs separately with `mise run perf:measure -- <label>` and `perf:measure-load`.
