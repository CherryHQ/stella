...
- exactly one pool-external, serialized PostgreSQL control session per replica handles abort/input/config wakeups, health checks and the global pull/WebSocket advisory lock. Transaction/statement pooling must be rejected from explicit deployment configuration or avoided with a direct/session-pooled control DSN; a black-box affinity probe is necessary but cannot prove an unknown proxy will never rebind a statement. Session loss immediately cancels listeners; graceful drain stops listeners before releasing leadership; notifications are wakeups repaired by heartbeat, reconnect and full scan;
