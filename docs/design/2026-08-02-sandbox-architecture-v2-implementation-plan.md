...
- [x] A destructive user, group, or Agent deletion fails without DB mutation if fence-lease acquisition fails. Deterministic cold-runner tests prove an admission already constructing a WorkspaceView completes before deletion, deletion then fences and commits without deadlock, and later admission fails. Cancellation while waiting on sorted Service or publication barriers releases partial acquisition. Concurrent production `SyncAgent` starts publish one Service, close the loser while structurally fenced, and synchronize the winner to the newest snapshot. A published-Service sync rechecks exact map identity and shutdown state after admission-barrier acquisition; disable/removal plus re-enable, replacement, and shutdown drift tests prove retired Services are never refreshed and the exact enabled winner receives the newest durable snapshot. Removal, shutdown, and Agent commit keep exact Services discoverable and fenced through terminal Runtime close; transaction rollback keeps the Agent Service published and usable. Physical bytes and inodes remain unchanged.
