...
- **What landed** — Provider-neutral Filesystem plus local/`none`/Docker `stella-fs` conformance; typed Home consumer and asset cutover; managed Skill publication, migration, marker, Home authority, logical telemetry, and retained-revision observability. Public host-layout contracts are removed and physical layouts are provider-private. Active project Skill, delegate, prompt, and relative tool paths use provider-owned canonical Filesystem coordinates. Filesystem and Linux bwrap now preserve the same deepest-mount and read-only authority for roots, nested mounts, masked aliases, and missing roots. Vision/Xberg accepts validated immutable bytes and stages only daemon-owned `0600` files. PostgreSQL mutable Skill current-state readers, writers, and changelog lifecycle are removed; migration, marker, archive, backup, and logical usage remain.
- **Acceptance results** — All remaining Phase 2 task and Acceptance checkboxes are complete. Deterministic generation was verified by identical hashes across repeated `mise run generate` on the dirty generated diff; DB and generation gates from prior Phase 2 slices passed. Final gates passed: `mise run format`, `mise run build`, serial full `mise run test`, focused race coverage across sandbox/providers and skills/reflect/pluginhost/skillaccess/server/stellad, the exact full-repository `mise run test:coverage:race`, Linux/Windows cross-compilation, `git diff --check`, supported-host `mise run system-test` with temporary `STELLA_HOME`, and final independent blocker reviews marked `APPROVE`. Before the stack rebase, GitHub Format and Linux race-plus-coverage Test checks passed at `sandbox/host-path-cleanup@d3b2d13be`. Earlier real Docker conformance and retained-revision/migration fixtures are part of the Phase 2 acceptance evidence.
- **Decisions made during implementation** — Home mutation happens first, followed by exact logical telemetry coordination. If telemetry fails after visible Home mutation, the outcome is unknown and is never retried. Filesystem path strings do not identify their coordinate system: each Provider projects its own execution paths and returns its canonical working directory without exposing a physical source. Explicit `Host=nil` prompt compatibility remains operator/test-local only and is never an active-runner fallback. Windows daemon-side Xberg fallback fails closed; the sandbox Xberg CLI remains supported.
- **Surprises / gotchas** — The active runner prompt rendered before Session. Vision initially accepted an arbitrary daemon host path. Final review found orphan Reflect telemetry after generic edit/file-delete/convert/delete paths; it was fixed across every generic path. Linux CI then exposed `/tmp` host/canonical ambiguity, bwrap mount-order and read-only mismatches, an admission-test lease lifecycle bug, and unsynchronized Xberg descendant readiness; each finding reopened acceptance until the exact GitHub gate passed.
...
#### Post-acceptance stack rebase

- **Rebased stack** — On 2026-08-08 the complete Draft stack was rebased onto `main@c8786afff`: `#829@fbb69692e` → `#831@f5caabb81` → `#862@7c2f6afb7` → `#886@fada961ee` → `#888@db544b8f0` → `#897@32af40841` → `#928@7e79db4b6`. Every PR retains its original stacked base and Draft state. The rewritten branches were pushed bottom-up with exact pre-rebase SHA leases.
- **Mainline reconciliation** — The rebase preserved current-main guest admission, turn cancellation, durable guest identity, security context, telemetry, provisioning, and modular Web Agents behavior. Channel attachment ingress remains typed-Home authority while retaining registration, unregistration, local-command admission, group publication, and Feishu thread routing. Attachment bytes are downloaded only after `AssetSaveAdmitter` approves identity and Home ingress. Skill policy and expected-digest CAS were ported into the current inspectors rather than restoring obsolete UI components.
- **Schema reconciliation** — Mainline owns migration `90000000000006_backfill_channel_group_allowlists`; the Skill migrations are now `90000000000007_add_skill_home_usage_identity` and `90000000000008_add_logical_reflect_skill_usage`. No migration was replaced or reordered across an applied identifier.
- **Verification** — At the rewritten top head, `mise run format`, `mise run build`, `mise run test`, `mise run build:web`, and `git diff --check` exited 0; the full Go suite and all 197 Web tests passed. Per V's direction, pending GitHub CI is not a handoff gate for starting Phase 3.
- **Phase boundary** — The rebase introduced no new authority or phase-order change. Phase 3 starts from `sandbox/host-path-cleanup@7e79db4b6`; Docker Compose multi-replica remains closed until Phase 3 durable execution acceptance, and Kubernetes remains closed until the Phase 4 Compose gate passes.
