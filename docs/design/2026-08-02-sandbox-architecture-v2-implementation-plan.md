...
> **Approved Phase 1 replacement (2026-08-11):** PostgreSQL Home catalogs, Store lifecycle, migration/cutover, tombstones, and purge are removed from the active plan. Phase 1 is one replica and one POSIX `STELLA_HOME`; PostgreSQL owner rows remain identity/authorization authority, deterministic filesystem paths are data/layout authority, and one `WorkspaceManager` materializes validated roots. Owner deletion lock order is process lifecycle fence → local owner gate → existing DB transaction, preserving bytes and inodes. Global Agent filesystem entries reserve IDs. Multi-replica, S3 authority, generations, and leases require a future redesign. Conflicting historical checklist text below is retained only as superseded decision history.
...
### 1. Phase 1 uses a local workspace manager

`internal/home.WorkspaceManager` owns Phase 1 materialization beneath one trusted POSIX `STELLA_HOME`. PostgreSQL owner rows authorize deterministic paths; the filesystem owns layout and bytes. Missing roots for live owners are created, unsafe or wrong-kind roots fail closed, and deletion fences execution before deleting the owner while preserving bytes and inodes. There is no Home registry, Store lifecycle, registration, ready state, tombstone, or migration metadata in Phase 1.
...
Tradeoff: this is deliberately a trusted-host, single-replica POSIX contract. Multi-replica, Kubernetes, S3, generations, leases, placement, and physical purge require a future redesign.
...
| `internal/db/queries/`                                                             | Existing owner queries support Phase 1; later phases add `ctx_session_sandbox.sql`, `ctx_agent_run.sql`, and `ctx_chat_input.sql`, evolve Session inbox queries, narrow group queries, and guard transcript/source updates.                                                            |
| `pkg/db/sqlc/`                                                                     | Regenerated only through `mise run generate`; never hand-edited.                                                                                                                                                                                                                       |
| `internal/home/`                                                                   | Phase 1 local `WorkspaceManager`, owner deletion gate, deterministic paths, and POSIX no-follow materialization. Future distributed/provider storage is a redesign.                                                                                                                    |
...
| 1.1 | `storage/typed-homes`           | Phase 1 vertical slice: sole local materializer, durable-owner validation, deterministic typed paths, consumer routing, deletion fence, and byte/inode retention                            | 0.1        |
...
### Phase 1: Local workspace ownership
...
- [x] Keep PostgreSQL user/group/Agent rows as identity and authorization authority; add no Home schema or migration metadata.
- [x] Implement one injected `internal/home.WorkspaceManager` as the sole production materializer beneath a real POSIX `STELLA_HOME`.
- [x] Use deterministic user/group/Agent paths, create missing roots only for live durable owners, reject symlinks/non-directories/unsafe IDs/root replacement, and retain existing bytes and inodes.
- [x] Reserve a global Agent ID when any filesystem entry exists at its deterministic Agent root.
...
- [x] Route runner/project/workspace setup through `WorkspaceManager`; remove direct production typed-root `mkdir` APIs.
- [x] Implement destructive owner deletion with lock order lifecycle writer fence → local owner gate → existing DB transaction. Preserve all physical bytes and inodes; post-delete views fail durable-owner validation.
- [x] Add user/group isolation, same-raw-ID collision, no-data-move, concurrent materialization, deletion lifecycle, byte preservation, path safety, lock cancellation, and Agent ID occupancy tests.
...
- [x] A destructive user, group, or Agent deletion fails without DB mutation if lifecycle-fence acquisition fails. Deterministic cold-runner tests prove an admission already constructing a WorkspaceView completes before deletion, deletion then fences and commits without deadlock, and later admission fails. The context-aware shared/exclusive gate cleans canceled waiters and gives queued writers progress under continuing readers. Concurrent `SyncAgent` calls, start/Close, both disable/re-enable orderings, raw stale Service admission, slow Runtime close, and owner commit/rollback tests prove lifecycle reconciliation reads durable state only after exclusive acquisition, publishes at most one current Service, keeps it map-visible through terminal close, and installs the newest committed snapshot. Physical bytes and inodes remain unchanged.
...
- **What landed** — a local `WorkspaceManager` as sole materializer, durable-owner validation, deterministic typed paths, POSIX no-follow checks, consumer injection, global Agent-ID filesystem occupancy, and owner fence/delete with bytes and inodes preserved. No Home catalog, Store registry, registration, ready state, tombstone, or storage migration landed.
- **Acceptance results** — focused tests cover typed path isolation, durable-owner failure without filesystem mutation, deterministic lock ordering and cancellation, path/symlink/wrong-kind rejection, Agent ID occupancy, owner deletion fencing, byte/inode retention, rollback, post-delete rejection, and cold-view lifecycle ordering. Record only commands actually rerun for the current head.
- **Decisions made during impl** — one injected `home.WorkspaceViewer` is mandatory with no production fallback. The manager uses bounded context-cancellable owner stripes and a pinned no-follow POSIX root; it is closed during server shutdown. Every production Service shares one process-local lifecycle gate. Future multi-replica execution and non-POSIX authority require a new distributed storage and fencing contract.
- **Surprises / gotchas** — deleting a durable owner while preserving its files intentionally leaves an occupied path. Workspace access still fails because PostgreSQL remains authority, and an Agent root entry of any kind reserves that ID.
- **What changed from this plan** — Phase 1 is a single-replica POSIX contract and does no physical destruction or Store cutover. Distributed placement, Kubernetes, S3, generations, leases, migration, and purge must be redesigned rather than built on a nonexistent registry.
