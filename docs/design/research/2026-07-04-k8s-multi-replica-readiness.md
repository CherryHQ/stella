...
| Recally 文章正文     | `$STELLA_HOME/library/{u}/articles/...`(DB 只存相对路径) | `internal/library/recally/files.go:24-31`          | **持久用户数据,DB 行在、文件只在一个 pod 上**                                                            |
...
| M2  | OAuth flow 状态在内存                | `internal/connections/oauth/store.go:5-9`(注释自认)                                                                                                        | auth-code callback / device-flow poll 打到另一副本 → "unknown or expired flow"。callback 不受 LB stickiness 保证,必须 DB 化(带 TTL)                                                                                                                |
