...
チャネルプラグインとプロバイダープラグインは管理パネル（`anna --open`）で設定します。管理パネルは`settings_plugins`テーブルに書き込み、トークン、キー、プラグイン固有の設定を管理するUIを提供します。プロバイダープラグインはプロバイダーページに動的に表示されます——新しいプロバイダープラグインを追加すると、管理UIのドロップダウンに自動的に表示されます。
