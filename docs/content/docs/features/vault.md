...
Anna also ensures a per-user `ANNA_TOKEN` exists before sandbox env injection. This token is generated by Anna, stored encrypted in the vault for sandbox use, and can authenticate requests to the admin API with `Authorization: Bearer <token>`.
...
- Must not start with `ANNA_`, `LC_`, or `XDG_`, except for Anna-managed `ANNA_TOKEN`.
