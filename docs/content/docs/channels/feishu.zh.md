...
2. **机器人回复一张交互卡片** — 包含"授权"按钮
3. **用户点击按钮** — 在浏览器中打开飞书 OAuth 授权页面
4. **用户同意授权** — 在 OAuth 页面上批准权限请求
5. **用户复制授权码** — 从重定向页面复制
6. **用户发送 `/auth <code>`** — 将授权码发回机器人（例如 `/auth abc123def456`）
...
1. 进入飞书开放平台控制台的**安全设置**
2. 添加**重定向 URL** — 由于用户手动复制授权码，可以是任意 URL（例如 `https://open.feishu.cn/`）
