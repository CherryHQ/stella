...
除了聊天自动注册，你还可以为管理后台启用**使用飞书登录**功能。这样员工就可以用飞书工作账号登录 anna，而不需要单独的本地密码。

### 工作原理

1. 用户在 `/login` 页面点击「使用飞书登录」。
2. anna 将用户重定向到飞书 OAuth，仅请求最小权限（仅认证，不包含 CLI 权限）。
3. 授权后，飞书将用户重定向回 anna，并携带临时授权码。
4. anna 用授权码换取用户信息（`union_id`、姓名、邮箱、tenant_key）。
5. anna 查找是否已有用户绑定了该飞书身份：
   - 如果找到且账号有效：完成登录并重定向到管理后台。
   - 如果未找到且启用了自动注册：创建新用户并完成登录。
   - 如果未找到且未启用自动注册：显示错误，提示用户联系管理员。
...
> **注意：** 同一时刻只能有一个实例启用 `enable_login: true`。如果多个实例同时启用，飞书登录将被禁用，直到你仅配置一个为止。

### 安全说明

- 飞书网页登录仅请求**最小权限**（仅用于认证），不请求 `offline_access` 或工作区权限（如日历、文档、云盘等）。
- 飞书网页登录**不会**在你的保险箱中存储长期凭证，它仅创建一个网页会话。要在 agent 会话中使用 `lark-cli`，你仍需在「凭证」页面单独连接 CLI 凭证。
- 用户身份通过 `union_id`（优先）或 `open_id`（旧版兼容）解析。邮箱**不会**用于关联账号——如果本地用户有相同的邮箱但没有飞书身份，不会自动关联。
- 如果配置了 tenant_key，会进行校验。外部飞书租户的用户无法登录或被自动注册。
- 引导安全规则禁止通过飞书登录创建第一个管理员。你必须先注册一个本地管理员。

### 与聊天自动注册的区别

| 功能 | 聊天自动注册 | 网页登录 |
|------|-------------|----------|
| 触发方式 | 用户向机器人发消息 | 用户点击「使用飞书登录」|
| 租户检测 | 启动时自动探测 | 必须显式配置 |
| 创建用户 | 是（如果启用） | 是（如果启用且租户匹配）|
| 创建网页会话 | 否 | 是 |
| 存储 CLI 凭证 | 否 | 否 |
...
| `enable_login`       | 启用**使用飞书登录**网页登录功能。同一时刻只能启用一个实例。   |
