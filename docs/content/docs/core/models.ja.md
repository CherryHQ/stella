...
`model_strong`と`model_fast`の両方は、設定されていない場合は`model`にフォールバックします。これらは管理パネル（`anna onboard`）を通じてエージェントごとに設定します。

## プロバイダー設定

プロバイダーは管理パネル（`anna onboard`）を通じて設定されます。各プロバイダーは、オプションのAPIキーとベースURLを持つ`settings_providers`テーブルに保存されます。
