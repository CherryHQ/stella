...
## サンドボックスバックエンドとしての Docker

annaをDockerコンテナ内で実行すること（上記参照）と、エージェントツール実行のサンドボックスバックエンドとしてDockerを使用することは別の概念です。両者を組み合わせることも可能（Docker-in-DockerまたはSocketのマウント）ですが、それぞれ独立して有用です。

### `docker` サンドボックスバックエンドを優先する場合

- **Windows**：`boxsh` はLinux/macOSのみ対応。`docker` バックエンドはDocker Desktopを通じてWindowsユーザーに真の隔離境界を提供します。
- **カスタムツールチェーン**：ホストと異なる特定のPython/Node/Goバージョンや、クリーンなLinuxユーザースペースが必要な場合。
- **副作用の隔離**：再現可能なファイルシステム状態が必要で、エージェントスクリプトによるホストレベルの副作用を避けたい場合。

### トレードオフ

- **起動レイテンシ**：コンテナのウォームスタートで約200ms、初回プル時は約1–3s。
- **バインドマウントパフォーマンス**：macOS/WindowsのDocker Desktopでは、バインドマウントのファイルシステム操作がネイティブディスクより5–20倍遅くなります。大量の読み書きが発生するワークフローではこれらのプラットフォームで `docker` バックエンドを避けてください。
- **コピーオンライト分離なし**：`boxsh`（overlayfsを使用）とは異なり、dockerバックエンドはoverlayベースのCOWを提供しません。暴走したスクリプトがマウントされたワークスペースを変更・破損させる可能性があります。

`sandbox.docker` 設定キーとJSONの例については、[設定ガイド](/docs/getting-started/configuration)を参照してください。
