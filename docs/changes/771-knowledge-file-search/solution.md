...
一次工具调用只接收一条 query。同一条用户消息触发的 Agent 执行中最多调用两次：第一次使用初始 query，仅在结果不足时才允许改写 query 再调用一次。第二次之后，模型必须使用已有证据回答或明确说明证据不足，不能继续检索。提示词表达这项策略，Agent 运行时同时执行硬上限；下一条用户消息重新计数。V1 不接收 query 数组，也不在后端进行多查询融合。

服务端对 query 做空白和标点规范化。规范化后没有有效文字或数字时返回无结果，不执行全库扫描。query 最大长度为 500 个 Unicode 字符。
...
- 同一条用户消息最多实际执行两次 knowledge.search；第三次尝试返回工具错误，下一条用户消息恢复两次额度。
