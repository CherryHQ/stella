      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            <svg className="w-3.5 h-3.5 shrink-0 text-muted-foreground/50" fill="none" viewBox="0 0 24 24" strokeWidth="1.8" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
            </svg>
            <span className="text-xs font-mono text-muted-foreground/80 truncate">{fileEditor.path}</span>
            {fileEditor.language && (
              <span className="text-[9px] border border-border rounded-full px-1.5 py-0.5 font-mono">{fileEditor.language}</span>
            )}
          </div>
          <div className="flex items-center gap-1 shrink-0">
            {fileEditor.language === "markdown" && (
              <Button
                ...
                className={cn("text-xs", fileEditor.previewMd ? "text-primary" : "text-muted-foreground")}
              >
                Preview
              </Button>
            )}
            <span className="text-[10px] font-mono text-muted-foreground/30 select-none hidden sm:block">⌘S</span>
            <Button
              size="xs"
              ...
              className="gap-1"
            >
              {fileEditor.saving && <div className="w-3 h-3 border border-current/30 border-t-current rounded-full animate-spin" />}
              Save
            </Button>
            <Button variant="ghost" size="xs" onClick={onClose} className="px-2 text-muted-foreground/60 hover:text-foreground">✕</Button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {!fileEditor.previewMd && !fileEditor.loading && (
            <textarea
              ...
              style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, lineHeight: 1.6, tabSize: 2 }}
              spellCheck={false}
              autoComplete="off"
              autoCorrect="off"
            />
