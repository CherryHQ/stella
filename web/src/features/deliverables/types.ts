...
export type ItemKind = "deliverable" | "schedule";

export type AutomationItem =
  | { kind: "deliverable"; id: string; data: ComponentsDeliverable }
...
export function parseItemKey(key: string): { kind: ItemKind; id: string } | null {
  const m = key.match(/^(deliverable|schedule):(.+)$/);
  if (!m) return null;
...
/** A root deliverable needs you while blocked (verdict/dep/budget). */
export function deliverableNeedsYou(d: ComponentsDeliverable): boolean {
...
export function classifyItem(item: AutomationItem): Section {
  ...
  const d = item.data;
  if (deliverableNeedsYou(d)) return "needs-you";
