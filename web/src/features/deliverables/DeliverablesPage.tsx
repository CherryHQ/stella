...
import { AgentChip } from "@/features/deliverables/DetailShell";
import {
  blockReasonLabel,
  type DisplayStatus,
  deliverableNeedsYou,
...
import { deleteDeliverable, unarchiveDeliverable } from "@/lib/api-client";
import type { ComponentsDeliverable } from "@/lib/api-client/types.gen";
import { useI18n } from "@/lib/i18n";
import type { MessageKey } from "@/lib/i18n/messages";
import {
  DELIVERABLES_PAGE_SIZE,
  deliverableCountsOptions,
  deliverablesPageOptions,
} from "@/lib/queries/deliverables";
import { formatTime } from "@/lib/time";
import { cn } from "@/lib/utils";

export type DeliverablesView = "triage" | "board" | "table";
type DeliverablesMode = "active" | "history" | "archived";
type StatusFilter = "all" | DisplayStatus;

const VIEWS: DeliverablesView[] = ["triage", "board", "table"];
const VIEW_LABEL: Record<DeliverablesView, MessageKey> = {
  triage: "deliverables.viewTriage",
  board: "deliverables.viewBoard",
  table: "deliverables.viewTable",
};
const VIEW_ICON: Record<DeliverablesView, typeof Inbox> = {
  triage: Inbox,
  board: Columns3,
  table: TableIcon,
};

// Terminal lifecycles close a deliverable out of the active set.
const TERMINAL_LIFECYCLES = new Set(["accepted", "rejected_final", "abandoned", "cancelled"]);
const isTerminal = (d: ComponentsDeliverable) => TERMINAL_LIFECYCLES.has(d.lifecycle);
...
export function DeliverablesPage() {
  ...
  const cur: DeliverablesView = VIEWS.includes(view as DeliverablesView)
    ? (view as DeliverablesView)
    : "triage";
  const mode: DeliverablesMode =
  ...
  const { data: counts } = useQuery(deliverableCountsOptions(agentId));
  ...
  const rows = useMemo(() => {
    const all = pageData?.deliverables ?? [];
  ...
  const patch = useCallback(
    (next: Record<string, unknown>, replace = false) =>
      void navigate({
        to: "/agents/$agentId/deliverables/all",
        params: { agentId },
  ...
  const setView = useCallback((v: DeliverablesView) => patch({ view: v }), [patch]);
  ...
  const setMode = useCallback(
    (m: DeliverablesMode) => patch({ mode: m, page: 1, status: undefined }),
    [patch],
  );
  const openDeliverable = (d: ComponentsDeliverable) =>
    void navigate({
      to: "/agents/$agentId/deliverables/$deliverableId",
      params: { agentId, deliverableId: d.id },
    });

  // History archives the selected terminal deliverables; the archived view
  // restores them. The server already scopes each mode, so every selected row on
  // the page is actionable. Active deliverables are never bulk-actionable.
  ...
  const runBulk = useCallback(async () => {
    ...
    try {
      await Promise.all(
        ids.map((id) =>
          mode === "archived"
            ? unarchiveDeliverable({ path: { id }, throwOnError: true })
            : deleteDeliverable({ path: { id }, throwOnError: true }),
        ),
      );
      await Promise.all([
        qc.invalidateQueries({ queryKey: ["deliverables-page"] }),
        qc.invalidateQueries({ queryKey: ["deliverables-counts"] }),
  ...
  useEffect(() => {
    ...
    setHeaderActions(
      <div className="flex items-center gap-1">
        ...
        >
          <History />
          <span className="max-sm:hidden">{t("deliverables.modeHistory")}</span>
        ...
        >
          <Archive />
          <span className="max-sm:hidden">{t("deliverables.modeArchived")}</span>
    ...
    <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-border bg-card p-3 sm:flex-row sm:items-center">
      ...
      <Select value={status} onValueChange={(value) => onStatusChange(value as StatusFilter)}>
        <SelectTrigger size="sm" className="w-full sm:w-44">
          <SelectValue placeholder={t("deliverables.statusAll")} />
        </SelectTrigger>
        <SelectPopup>
          <SelectItem value="all">{t("deliverables.statusAll")}</SelectItem>
          {statusOptions.map((s) => (
      ...
      <div className="flex items-center gap-2 sm:ml-auto">
        <span className="font-mono text-xs text-muted-foreground">
          {t("deliverables.filteredCount", { count: total })}
        </span>
        {mode !== "active" && (
          <Button
            variant="outline"
          ...
          >
            {mode === "archived"
              ? t("deliverables.restoreSelected", {
                  count: selectedActionableCount,
                })
              : t("deliverables.archiveSelected", {
                  count: selectedActionableCount,
                })}
          </Button>
        )}
...
function Empty() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <p className="text-sm font-medium text-muted-foreground">{t("deliverables.empty")}</p>
      <p className="mt-1 max-w-xs text-xs text-muted-foreground">{t("deliverables.emptyDesc")}</p>
    </div>
  );
}
...
function FilteredEmpty() {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-border py-16 text-center">
      <p className="text-sm font-medium text-muted-foreground">{t("deliverables.noMatches")}</p>
      <p className="mt-1 max-w-sm text-xs text-muted-foreground">
        {t("deliverables.noMatchesDesc")}
      </p>
    </div>
  );
}

// One-line attention hook a deliverable's lifecycle/block_reason earns it.
function hookText(t: TFunction, d: ComponentsDeliverable): string | null {
  if (d.lifecycle === "blocked") {
    if (d.block_reason === "needs_verdict") return t("deliverables.hookNeedsVerdict");
    if (d.block_reason === "budget_exhausted") return t("deliverables.hookBudget");
    if (d.block_reason === "dep") return t("deliverables.hookDep");
    return blockReasonLabel(t, d);
  }
  if (d.lifecycle === "accepted")
    return t("deliverables.acceptedAt", {
      time: formatTime(d.accepted_at ?? d.updated_at),
    });
  if (d.lifecycle === "rejected_final") return t("deliverables.hookRejected");
  if (d.lifecycle === "abandoned") return t("deliverables.hookAbandoned");
  if (d.lifecycle === "cancelled") return t("deliverables.hookCancelled");
  return null;
}

const byUpdatedDesc = (a: ComponentsDeliverable, b: ComponentsDeliverable) =>
  new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime();

interface ViewProps {
  rows: ComponentsDeliverable[];
  onOpen: (d: ComponentsDeliverable) => void;
  selected: Set<string>;
...
function Triage({ rows, onOpen, selected, onSelect }: ViewProps) {
  const { t } = useI18n();
  const needs = rows.filter(deliverableNeedsYou).sort(byUpdatedDesc);
  const prog = rows.filter((d) => !deliverableNeedsYou(d) && !isTerminal(d)).sort(byUpdatedDesc);
  ...
  return (
    <>
      <Section label={t("deliverables.secNeedsYou")} arr={needs} />
      <Section label={t("deliverables.secInProgress")} arr={prog} />
      <Section label={t("deliverables.secClosed")} arr={closed} dim />
    </>
  );
}
...
function Row({
  ...
  const needs = deliverableNeedsYou(d);
  ...
  return (
    ...
    >
      <Checkbox
        ...
        aria-label={t("deliverables.selectItem")}
      />
      <button
        type="button"
        onClick={() => onOpen(d)}
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            {hook ?? d.intent ?? t("deliverables.progressHint")}
          </div>
        </div>
...
const BOARD_COLS: {
  ...
  match: (d: ComponentsDeliverable) => boolean;
}[] = [
  {
    labelKey: "deliverables.colPlanning",
    status: "draft",
    match: (d) => d.lifecycle === "draft" || d.lifecycle === "ready",
  },
  {
    labelKey: "deliverables.colRunning",
    status: "active",
    match: (d) => d.lifecycle === "active",
  },
  {
    labelKey: "deliverables.colNeedsYou",
    status: "review",
    match: deliverableNeedsYou,
  },
  {
    labelKey: "deliverables.colClosed",
    status: "accepted",
    match: isTerminal,
  },
];
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-border bg-muted/50 text-left font-mono text-[10.5px] text-muted-foreground">
            ...
            <th className="w-[120px] px-3.5 py-2.5 font-semibold">{t("deliverables.colStatus")}</th>
            <th className="px-3.5 py-2.5 font-semibold">{t("deliverables.colTitle")}</th>
            <th className="w-[160px] px-3.5 py-2.5 font-semibold">
              {t("deliverables.colAttention")}
            </th>
            <th className="w-[150px] px-3.5 py-2.5 font-semibold">{t("deliverables.colAgent")}</th>
            <th className="w-[90px] px-3.5 py-2.5 font-semibold">{t("deliverables.colUpdated")}</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((d) => {
...
function Pager({
  ...
  return (
    <div className="mt-5 flex items-center justify-between gap-3">
      <span className="font-mono text-xs text-muted-foreground">
        {t("deliverables.pageStatus", { page, totalPages, total })}
      </span>
      <div className="flex items-center gap-2">
