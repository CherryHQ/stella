...
import { DeliverableForm } from "@/features/deliverables/DeliverableForm";
import {
  SchedulePicker,
  isScheduleValid,
  emptySchedule,
  scheduleFromString,
  type ScheduleValue,
} from "@/features/deliverables/SchedulePicker";
...
type Mode = "deliverable" | "schedule";
...
export function DeliverableNewPage() {
  const { t } = useI18n();
  const { agentId } = useParams({ from: "/_app/agents/$agentId/deliverables/new" });
  const { project_id: projectId } = useSearch({ from: "/_app/agents/$agentId/deliverables/new" });
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [mode, setMode] = useState<Mode>("deliverable");
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-[640px] flex-col overflow-y-auto px-6 py-8">
      <div className="mb-5">
        ...
        >
          <ToggleGroupItem value="deliverable">{t("deliverables.new")}</ToggleGroupItem>
      ...
      {mode === "deliverable" ? (
        <DeliverableForm
          ...
          projectId={projectId}
          onCreated={(d) => {
            void queryClient.invalidateQueries({ queryKey: ["deliverables-page"] });
            void queryClient.invalidateQueries({ queryKey: ["deliverables-counts"] });
            void navigate({
