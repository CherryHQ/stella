...
PostgreSQL is the authority for Home identity and lifecycle; the configured Store is the authority for its file bytes. A `ready` registry row never causes Stella to recreate a missing root. Resolution fails when the root is missing, is not a directory, or is a symbolic link. LocalStore retains a directory pin through each bounded revalidation interval and detects replacement during that interval, but it does not provide durable inode identity against trusted host-side replacement across operations or process restarts. Run restore, storage migration, and Home-root cleanup only while Stella is stopped or all consumers are fenced, and restore the Home bytes and PostgreSQL metadata from a consistent backup.
