...
Uploaded user assets have the same ephemeral-disk problem as the database. By default the local filesystem under `STELLA_HOME` is the durable authority for assets — correct for a single node. Set `STELLA_REQUIRE_SHARED_ASSETS=1` when you run more than one replica (the Helm chart sets it automatically from the replica count): startup then fails fast unless a shared object store (`STELLA_BLOB_S3_*`) is configured, so an upload can never land only on one pod's disk and vanish on reschedule. This is a deployment-topology decision, independent of the database mode — a single-replica container with an external database but a persistent asset volume leaves it unset and keeps the local filesystem authority.
...
| `STELLA_REQUIRE_SHARED_ASSETS`   | No                        | Fail startup unless a shared object store (`STELLA_BLOB_S3_*`) is configured; set `1` for multi-replica deployments (the Helm chart derives it from the replica count). Independent of the database mode |
