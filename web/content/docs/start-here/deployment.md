...
The container runs as `nonroot` user. Mount `$STELLA_HOME` (usually `~/.stella`) to preserve durable Principal and Agent Homes, including mutable Skill catalogs and revisions, migration archives, mutable assets, and project Skills. PostgreSQL remains required for its own state, but mutable Skill content is in Home storage. Release-provided builtins come from the image's immutable bundle, not the host. You can set `STELLA_HOME` to change the data directory inside the container. The `--security-opt seccomp=unconfined` flag is required for the local sandbox backend (bwrap) to call `unshare(2)` inside the container.
...
A legacy Skill deployment must also migrate before the new server starts. Startup verifies the strict Skill Home authority marker and residual legacy PostgreSQL current state. Enter maintenance mode, stop all legacy Skill writers, create and verify a PostgreSQL backup, run the dry run, resolve every unsupported-item or collision report, then run the real migration. Both runs require all three confirmation flags; use `stellad storage migrate-skills --help` for syntax. The migration is idempotent, no-replace, and digest-verified; it preserves canonical metadata and writes migrated legacy PostgreSQL files as `0644`; it does not guess extensions or invent an executable bit. It archives deprecated/changelog data in hidden Home storage, migrates logical Reflect usage, and never deletes source PostgreSQL rows or backups. See [Skills and derived cache](/docs/start-here/storage#skills-and-derived-cache).
...
| `~/.stella/agents/{agent-id}/.agents/skills/` | Durable typed Home `system_agent` Skill catalog and managed revisions                         |
...
Back up PostgreSQL and every durable Home store, including mutable Skill catalogs, managed revisions, hidden migration archives, mutable asset trees, and project Skills. PostgreSQL contains configuration, message history, summaries, scheduler jobs, Home identity inventory, Agent Skill policy, logical Reflect usage/pair activity, and migration/audit/backup compatibility—not mutable Skill bytes or current state. With the embedded cluster, back up `~/.stella/postgres/` with the server stopped; `~/.stella/pg-runtime/` and `~/.stella/bundles/{revision}/` are derived and can be recreated. With an external server, use `pg_dump` against your `STELLA_DATABASE_URL` database. Keep and back up the configured object store because it holds immutable content-addressed session media.
