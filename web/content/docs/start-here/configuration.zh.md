...
所有配置都通过Web UI进行管理。使用 `stellad server` 启动服务器，然后在浏览器中打开 [http://localhost:25678](http://localhost:25678)。所有配置存储在 PostgreSQL 中——默认是托管在 `~/.stella` 下的内嵌集群，或在设置 `STELLA_DATABASE_URL` 时使用外部服务器。无需编辑任何配置文件。
