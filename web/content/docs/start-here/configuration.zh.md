...
结果有明确边界：Agent、Provider、插件和 MCP 列表最多返回 50 项，并在仍有更多项时说明。Library 列表按每页 1–100 项返回；Library 结果不会返回原始文件字节，托管 Skill 结果也不会返回文件内容。Agent 创建或更新托管 Skill 时传入完整的 Agent Skills 目录，因此 `references/`、`scripts/`、`assets/` 和其他配套资源仍可按需加载。Account、用户、Provisioning、渠道、Webhook、任意插件配置、Agent 工作区/沙箱设置和凭证变更仍须通过 Web UI 或 API 完成。
