...
添加提供商时，先选择模型目录条目。Stella 会自动填写适配器、基础 URL、提供商 ID 和显示名称，在创建记录前探测上游凭证，并把发现的模型保存为稀疏的启用覆盖项。使用提供商列表中的 **同步目录** 刷新共享的 models.dev 快照。模型开关会立即保存；如果其他浏览器标签页已经修改记录，页面会提示冲突，而不是覆盖其更改。

提供商页面使用 [models.dev](https://models.dev) 的内嵌快照展示模型名称、能力、上下文限制和参考价格。你可以为提供商实例关联目录厂商 ID，选择 `allow_all` 或 `allowlist`，并覆盖单个模型的字段。通过上游获取的模型只负责发现，目录元数据仍是上下文窗口和价格的回退来源。目录刷新使用 ETag 条件请求，上游不可用时回退到内嵌快照。
