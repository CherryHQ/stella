...
提供商页面使用 [models.dev](https://models.dev) 的内嵌快照展示模型名称、能力、上下文限制和参考价格。同名模型 ID 以及带 Provider 前缀的模型 ID 会自动匹配。如果上游使用了不同的模型 ID，展开该模型后可以搜索并选择它应继承的 Catalog 模型；匹配后会自动填充有效元数据和价格，但不会把继承字段复制进本地配置。你也可以明确选择不匹配、使用 `allow_all` 或 `allowlist`，并只覆盖需要修改的字段。通过上游获取的模型只负责发现，目录元数据仍是上下文窗口和价格的回退来源。目录刷新使用 ETag 条件请求，上游不可用时回退到内嵌快照。
