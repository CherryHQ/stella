...
Set `STELLA_AGENT_TOOL_MODE=code` before starting `stellad server` to opt an installation into Code Mode. It replaces the provider-facing native tool catalog with one `code` tool. The model can search and describe the agent's existing tools before invoking them; authorization, hooks, audit, redaction, and tool lifecycle stay on the Stella side. Set `native` or remove the variable to use the established native tool-calling path again.
