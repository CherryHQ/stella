...
You can upload supported documents, spreadsheets, presentations, web pages, ebooks, and text or data files up to 25 MiB. PDFs with selectable text are parsed locally. When an administrator configures **Settings -> Vision**, Stella may send only the pages of a scanned PDF that require OCR to that provider; the provider may be third-party and may charge per request.
