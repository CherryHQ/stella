...
任务 UI 存在的原因很简单：聊天记录不是项目管理工具。在 Web UI 中，打开某个 Agent 后进入**任务**页签，就可以在同一处查看一次性 tasks、定时工作和 goals。Project 页面也会默认打开该 project 的任务列表。
...
- Review 状态。
- Goal 的子任务和汇总状态。
- Task 或 run 关联的对话 session。

当 task 因等待你而阻塞时，任务页会显示 agent 提出的问题。直接在页面上回答即可恢复执行——task 重新运行时会把你的回答交给 agent。如果是上游依赖失败，也可以在同一页面豁免该依赖。
