...
Use chat for context and decisions; watch execution on the agent's **Goals** tab. The goal detail page opens on its timeline, where you can see why work blocked and leave human guidance that authorizes one more retry when the block is not an upstream dependency. See [Goals](/docs/task-system/overview) for the full model.
