...
Everything else is invariant: `web/src/globals.css` (the `@theme inline` bridge and `@layer base`), CossUI components, and all feature code stay untouched. The landing page (`routes/index.css`) follows automatically via the `--stella-os-*` aliases.
...
3. **Verify:** `vp build`, then screenshot light + dark across chat, settings, and the landing page. Check against the new `web-design.md`: accent frequency, surface depth levels, contrast. Eyeball the known hardcoded surface (the Google brand color in `features/login/LoginPage.tsx`) for clashes.
