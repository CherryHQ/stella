...
已承诺的任务会同步一份到飞书任务清单「Stella」，作为日常执行入口：
提醒、子任务、评论，以及“我的任务”里的个人可见性。这个投影由
`.agents/skills/execution-sync/` 的 `execution-sync` skill 负责，在每周 Base
写入之后或任务的承诺状态变化时手动运行。

Base 记录始终是生命周期、优先级、里程碑和交付统计的唯一权威。执行任务不增加
任何规划字段：只携带 Base record id 作为幂等键、投影的优先级和里程碑，以及
GitHub Issue 链接。飞书任务永远不作为交付证据；只有 Base 验收通过后才勾选
父任务，同步会自动重开被提前勾掉的任务。
