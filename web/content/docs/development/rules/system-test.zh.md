...
- `github_webhook_ingress` —— 管理员为一个普通 owner 与已分配 Agent 签发 vault 支持的
  GitHub endpoint：原始签名 delivery 只启动一个固定持久 session，重复与无效签名都不会启动，
  确定性的 busy admission 会释放 claim，供一次成功的重投递使用。
...
- **FIFO 轮次**（`enqueueText`）—— 一个按到达顺序回放的有序队列；由 `chat_sse` 与
  `github_webhook_ingress` 使用。未脚本化的请求会让测试失败。
