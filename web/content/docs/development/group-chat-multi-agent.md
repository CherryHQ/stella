...
Caps and nudge bookkeeping live on `ctx_group_state`: `agent_chain_hard_limit` (8), `max_agent_posts_per_minute` (**10 per agent by default, configurable from 1 to 1000**), `max_replies_per_human_trigger` (5), `hold_limit` (3), plus `nudge_at`, `nudge_checked_at`, and `nudge_streak_count`.
