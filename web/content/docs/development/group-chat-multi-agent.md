...
A wake is local work, not a command to speak. Before a turn runs, the server applies only deterministic rules: chain, rate, and per-human-trigger caps first, with no bypass; then explicit member mentions and targeted nudges act, a mention of another member is silent, and an agent that already spoke in the current agent-only run stays silent unless a live work claim shows the run is not idle chatter. Any wake no rule addresses runs the turn. No model decides whether another model may speak.
