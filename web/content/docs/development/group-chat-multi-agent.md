...
- A running dispatch whose lease expires while holding a `result_message_id` retires as completed rather than resuming, so a heartbeat hiccup cannot resend a whole response on platforms that do not confirm chunks.
