...
收敛循环用昂贵的方式赚来一份好的分解——靠规划与返工。**工作流（workflow）**把这份赚来的
计划固化下来，让下一次运行直接跳过它。工作流就是一个**被冻结的 Goal**：它快照了某个**已验收**
复合目标的分解结果，存为一棵递归的 `FrozenPlan`（`{children, edges}` 可嵌套到任意深度），
外加根节点的验收契约、收敛策略、版本号，以及计划内容的哈希。

实例化一个工作流，是对常规流程的反转：它不再走 `创建 → 规划 → 分解`，而是从快照中**确定性地**
重建子树——每个被冻结的复合节点都在单个事务内被物化并激活，完全跳过规划器，然后交给与其他
Goal 相同的调度、验收、上卷和定时机制。下游没有任何环节知道这棵树是被冻结重建、而非规划出来的。

冻结是**逐节点**的，这正是它成为光谱而非开关的原因。带有冻结子计划的复合节点会确定性地执行其
子节点；**没有**子计划的复合节点则在实例化时交还给规划器（“半冻结”）。于是一个工作流可以把你
信任的部分钉死，把会变化的部分留给重新规划。

两个入口创建并运行工作流，都走工作流 HTTP API：

- **save-as** 冻结你拥有的已验收复合目标（`stella workflow save-as <goal-id>`），并原样保留
  边，使兄弟依赖在往返中不丢失。
- **instantiate** 按需重建一棵新的 Goal 树（`stella workflow instantiate <id>`），而
  **schedule** 挂一个调度任务，每次触发实例化一次运行（`stella workflow schedule <id> --cron …`）
  ——调度器的工作流分发分支调用同一条实例化路径，并把新的根 Goal 记录到该次运行上。

工作流是一个建立在核心实体**之上**的、可复用、可调度的产物——它没有引入任何新的运行时概念，
只是为既有的收敛循环提供了一个冻结的起点。
