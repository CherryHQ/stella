...
Abort intent and completion compete in PostgreSQL. The winning terminal transition records both the Run result and Session turn activity in one transaction. Normal Stop therefore persists `canceled` Session activity, and an old executor cannot overwrite a successor's activity.

Model EOF does not finish a Run. The source first commits its remaining durable work: the reply and routing handoff for a channel turn, a group reply's accept/stop decision, scheduler bookkeeping, or a Goal transition. These writes retain the Run's transaction fence. The source then finishes the Run; direct channel and group handoffs commit atomically with that terminal transition. A durable stop request outranks the requested terminal status. A source whose commit result is unknown cannot report successful completion; an abandoned live Run expires as interrupted.

External channel delivery has its own record and never holds the Run open for an acknowledgement. Live model output requires the execution lease; after the output handoff commits, the channel uses that record to authorize delivery. Every request records an attempt before contacting the platform. An attempted delivery without a confirmed result is unknown and is never resent automatically. Direct channel handoffs retain image, file, and reference payloads; attachment bytes are stored in PostgreSQL in chunks of at most 1 MiB. This adds database storage proportional to retained attachments. Remote delivery and attachment retention policy remain follow-up work in #1035.
