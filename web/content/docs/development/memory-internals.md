...
5. **User profile** — durable user notes. **Group turns replace this with `## Group Memory` (the shared group drawer) plus an optional `## Current Speaker` section** for the linked speaker's profile; the per-user profile is never rendered in a group turn. Group mode is keyed on the session having a `group_id`, not on group memory being non-empty.
...
For group turns the per-turn rebuild is a `GroupPromptFunc` that re-renders the full prompt with the current speaker; the cached group runner never holds speaker data, so one speaker's profile cannot leak into another speaker's turn. The speaker's profile is read at its own snapshot version, and the speaker's soul/constraints are intentionally not injected.
