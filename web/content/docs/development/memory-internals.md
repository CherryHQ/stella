...
- `legacy` keeps the old shared Blob prompt and compatibility behavior. An explicit `memory.read` of the well-known `profile` ref may fall back to the linked current speaker; other private reads still fail closed.
- `structured` does not register the model-facing `memory` tool for group turns. Current Group Facts are injected into the prompt and public history is already available through the group's per-Agent LCM, so the tool would add no group-owned data while risking private user recall.
...
For all group turns, current-speaker metadata is added per turn rather than cached in the runner. Structured mode keeps only public identity metadata and never injects or exposes the speaker's private profile.
