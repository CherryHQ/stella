...
An explicit destructive group or Agent delete tombstones Homes before removing the owner and durably enqueues one purge batch in the delete transaction. User deletion has the same internal lifecycle primitive, pending integration with the product account-delete flow. After commit, the process attempts to fence local cached execution; the shared River worker fences again, then takes a durable, exclusive, expiring claim before purging bytes idempotently. A crash or expired claim is recoverable by another worker. A failed physical purge remains `purge_failed` with audit data for operator retry. Assignment removal, member removal, Session archive, and Helm uninstall are not Home deletion operations. Attachments identify requested Home access but do not themselves confer authority. This is a single-replica boundary; multi-replica topology and cross-replica SessionSandbox fencing are future work.
