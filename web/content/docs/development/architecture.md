...
internal/
  ...
  agent/               Service, ServiceManager, session registry, runtime, runner factory
    ...
    sandbox/           Core sandbox tools (bash, vllm)
...
The core local-workspace tools run through a Docker sandbox backend. The `bash` tool executes via `Session.Exec` and is the only file-operation tool; its description carries the contract that dedicated read/write/edit schemas used to encode. `vllm` uses the mediated `Session.Files` capability with process-visible paths and is registered only when the deployment configured a vision model. Provider backing paths never enter the tool layer. Runner startup fails closed when Docker is unavailable.
