...
**Authorization.** Request handlers still run on the legacy `AuthInfo`/`authz.Identity` model; the new immutable `authz.Authority` is minted only in the trusted producer packages (`internal/auth`, `internal/credential`, `internal/authz`), enforced by a mint-boundary tripwire, and consumed today only by the `agentshadow` seam (advisory shadow comparison, deleted when the Agent vertical cuts over). Request body/path can never mint or overwrite an actor.
