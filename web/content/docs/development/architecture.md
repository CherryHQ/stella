...
internal/
  ...
  scheduler/           gocron service, heartbeat (agent uses scheduler_* tools; CLI for humans)
  skills/              Skills tools (skill_search/skill_list/skill_install; CLI for humans)
pkg/
...
| `task_*`      | Always                            | Durable tasks/goals (create/list/get/cancel/events/deps); ctx-scoped |
| `scheduler_*` | Always                            | Schedule jobs (add/list/remove); ctx-scoped                          |
| `skill_*`     | Always                            | Skill management (search/list/install); ctx-scoped                   |
| `vault_*`     | Vault key configured              | Encrypted secrets (list/set/delete; no get/keygen — never exposed)   |
| `oauth_*`     | Credentials service available     | Connect external services (providers/status/connect/disconnect)      |
| `share_*`     | Share service wired               | Public links for artifacts/articles (share_artifact/share_article)   |
| `notify`      | Gateway mode + channel configured | Send notifications via dispatcher                                    |

These ctx-scoped tools read agent/session/user/project identity from the Go context, so no identity arguments appear in their schemas. The equivalent `stella` CLI commands remain as the human/operator surface.
