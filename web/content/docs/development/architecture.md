...
Agent sends atomically link a durable input receipt to one target `AgentRun` and project its transcript input. The existing local FIFO waits for a busy owner; queue timeout, no capacity, or cancellation before admission leaves a failed, unlinked receipt. Recovery follows linked Run state without replaying a model or tool; legacy and unassociated receipts may receive append-only transcript recovery. PostgreSQL owns execution admission, while process-local guards provide fast rejection and cancellation. Nested calls carry depth and ancestry in context, reject cycles, inherit the root deadline, and share a 16-call root-turn budget across sibling and nested calls. Agent-originated input persists its actor and source Session; prompt rendering marks it as information rather than human authority. Synchronous Session turns never publish to an external channel implicitly, and inbox durability does not guarantee a reply after a crash. See [Run ownership and completion](/docs/development/agent-architecture#concurrency) for lease and terminal-state ordering.
