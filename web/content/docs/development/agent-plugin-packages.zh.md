...
Agent Plugin 读取器加载可移植包声明。运行时发现会把这些声明作为包含独立资源种类的完整包读取。单独读取包不会安装二进制文件、启用 Native
能力、创建 OAuth 连接或启动进程。
...
生成的 YAML 只是内嵌数据格式。安装包会把完整文件树发布到四个有类型的资源作用域
之一。编辑包会替换完整目录；复制包没有父级链接，不会跟随来源后续编辑。独立 Skill
和 MCP 文件仍是独立资源。

包声明不得包含 token、client secret、数据库 ID、Vault 定位信息或安装状态。凭据引用
只是公开定位符，OAuth grant 和连接状态保存在独立授权存储中。删除 MCP 文件不会
断开 OAuth，Disconnect 是单独的本地撤权动作，会撤销匹配的已存 grant、关闭连接并阻止迟到回调或刷新；不承诺远端 provider 一定撤权。

同一会话选中的包必须使用不同的环境变量名。两个包声明同名变量时，即使指定同一个
provider，也会在注入凭据前拒绝准备会话。运行时为每个 turn 捕获完整包，编辑在下一轮
生效；捕获身份仍匹配时可以复用 CLI 产物和远程连接。`none` sandbox 不提供可靠的进程
隔离。local backend 仍执行配置的沙箱策略，但不能证明脱离进程组的后代已经停止，因此保留的缓存和包字节只有在验证进程终止后才能清理。
