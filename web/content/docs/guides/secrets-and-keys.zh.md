...
1. 添加自己的 `user` 或 `user_agent` 密钥时，打开 **个人设置 → 连接与个人密钥库**；添加管理员管理的 `system` 或 `system_agent` 密钥时，打开 **管理控制台 → 部署资源 → 系统凭据**。
2. 在密钥库区域选择适用于全部智能体或单个智能体；页面已经固定了所有者。
...
- 必须以字母开头
- 最长 128 个字符
- 不能使用系统托管凭据名称，例如 `STELLA_TOKEN` 或 `GH_OAUTH`
