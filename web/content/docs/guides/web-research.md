...
If direct retrieval or extraction fails, WebFetch sends the already-validated public URL to Jina Reader and uses its Markdown response. This discloses the selected URL to Jina, but never sends provider credentials or URLs containing credential-like query parameters.
