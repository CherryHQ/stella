...
- **Redirect URI**: `<STELLA_BASE_URL>/auth/callback/mcp`. The base URL must be reachable by your browser, behind an ingress or a published port, not just inside the cluster. Existing registrations using `/api/mcp/oauth/callback` must be updated before reconnecting.
