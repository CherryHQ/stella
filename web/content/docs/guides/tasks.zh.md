...
- **任务页签** -- 打开某个 Agent 后选择**任务**，在同一处查看 tasks、定时工作和 goals。
- **Inbox** -- 需要你处理的任务（阻塞、待 review 或失败）会出现在侧边栏顶部的 Inbox 中，跨所有 Agent。
- **任务详情** -- 查看 readiness、事件历史、runs、blockers、reviews 和依赖。
- **Goal 详情** -- 查看 child tasks 和 goal 的汇总状态。
- **Project 任务列表** -- 打开 project 会先看到该 project 的 tasks；需要项目级持久对话或 workspace 时，再进入会话和文件页签。
