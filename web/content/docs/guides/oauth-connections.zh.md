...
- 权限范围按命名空间前缀分组（例如 `im:`、`docs:`），并支持搜索。粘贴多行列表可一次添加多个——会自动拆分、去除空白并去重。
- 差异栏显示相对于已保存覆盖值以及内置默认值的变化。非默认项标记为 **custom**；被移除的默认项仍然可见，因此删除是可逆的。
- **重置为默认**会清除覆盖值，恢复为内置列表。
- 当 Stella 升级新增了你的覆盖值未包含的默认权限范围时，合并提示会让你一键补齐。

覆盖值为空表示"使用内置默认值"。扩大请求的权限范围**不会**改变已签发的令牌：已连接用户必须重新连接才能授予新增的权限范围。
