...
- **默认权限**在用户首次连接时请求。
- **允许增量申请的权限**限定用户之后可添加的最大集合；超出允许列表的请求会在 Stella 联系 provider 前失败。

管理员可以分别用权限编辑器覆盖这两组设置：
...
保存后应用当前策略。默认权限是每个用户都必须具备的最小下限；每个用户的 desired scopes 等于这个下限加上该用户自行请求的额外允许权限。修改默认权限**不会**静默扩大已签发令牌，但受影响的用户需要重新连接。当工具需要另一个已允许的权限时，Stella 只会把它合并进该用户的 desired scopes，其他用户连接不受影响。

管理员收窄允许列表后，Stella 会从该用户下一次授权请求中移除已禁止的权限，并把连接标记为需要重新连接。OAuth provider 可能仍保留之前授予的权限；需要严格撤销时，请断开连接并在 provider 侧撤销授权。
