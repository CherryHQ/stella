...
> **开发 Stella 本身？** 不必手动 `docker run`。`mise run dev:api:docker` 会用本地构建的镜像拉起整套 `docker-compose.yml` 栈——`stellad` 加一个 `otel-lgtm` 边车,OTLP 已接到 `otel` 服务——无需任何额外配置,追踪与日志即刻上报。用 `docker compose down` 停掉。
