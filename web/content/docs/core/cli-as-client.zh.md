...
anna CLI 被有意做成轻量 REST 客户端。运行中的 `anna serve` 进程是**唯一**会
...
- **天然支持远端**：`ANNA_SERVER_URL=https://anna.example.com anna recally list`
  与本地用法走同一份代码。
...
Bearer token 鉴权要求 server 启动时设置了 `ANNA_VAULT_KEY`（token 材料经 vault
加密存储）。
