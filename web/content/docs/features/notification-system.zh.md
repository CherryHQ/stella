...
Stella 支持主动通知,因此代理、定时任务和其他内部触发器可以在不等待请求的情况下向用户推送消息。该系统使用多通道分发器将通知路由到一个或多个配置的通道(Telegram、QQ、微信,计划支持 Slack/Discord)。
...
当没有注册通知通道时,`notify` 工具不会暴露给代理。这避免了损坏的工具路径。
