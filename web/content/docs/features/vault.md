...
The vault requires a single master key, provided via the `STELLA_VAULT_KEY` environment variable. When this variable is not set, the vault endpoints return `503` and secrets are not injected.
...
Set the environment variable before starting stella:
...
On startup stella automatically generates an age keypair for every user that does not have one yet. No manual provisioning step is required when adding new users.
...
Stella writes the value to your vault without exposing it to the model or conversation
...
Stella also ensures a per-user `STELLA_TOKEN` exists before sandbox env injection. This token is generated by Stella, stored encrypted in the vault for sandbox use, and can authenticate requests to the admin API with `Authorization: Bearer <token>`.
...
- Must not start with `STELLA_`, `LC_`, or `XDG_`, except for Stella-managed `STELLA_TOKEN`.
- Must not equal or start with `PATH_`, `HOME_`, `USER_`, `SHELL_`, `LANG_`, `TERM_`, or `TMPDIR_`.

## Security model

Stella uses **envelope encryption**:

1. At startup a master X25519 keypair is loaded from `STELLA_VAULT_KEY`.
