...
sandbox sessions without manual authentication. Stella handles the OAuth device flow on
...
GitHub works out of the box. Stella uses the public GitHub CLI OAuth device-flow app,
...
empty unless you need an override: Stella derives it from the current Admin UI origin,
...
Inside any Stella channel session, ask the agent to connect a provider. It will call the
...
1. Open the Stella admin panel and navigate to your **Credentials** page.
2. Find the **OAuth CLI Credentials** section.
3. Click **Connect** next to the provider you want to link.
4. Stella starts a device flow and displays a verification URL and user code.
5. Open the URL in a browser, enter the code, and authorize.
6. Stella polls for completion. Once authorized, the token bundle is saved to your vault.
...
You do not need to duplicate that choice in `$STELLA_HOME/plugins.yaml`. The built-in
...
Only override `tool/lark-cli` in `$STELLA_HOME/plugins.yaml` if you need to change the
...
without any additional configuration. Stella injects the required auth env for each
session and prepends `$STELLA_HOME/bin` to `PATH`, so `gh` and `lark-cli` resolve
straight to Stella-managed binaries. For `lark-cli`, Stella injects
...
Feishu and Lark user access tokens expire after approximately **2 hours**. Stella
...
Stella will automatically restart the sandbox with the new token on the next message.

### Restart loses in-flight device flows

Pending device flows (started but not yet authorized) are held in memory. An Stella
process restart discards them. If Stella restarts while you are completing authorization
