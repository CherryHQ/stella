...
- has one canonical plugin ID such as `channel/telegram` or `provider/openai`
...
- `tool/lightpanda`: pinned headless-browser binary + prompt guidance, used by the `web` system skill
