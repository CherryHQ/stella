...
如果用户已经通过飞书 OAuth 登录过 Stella，那么该用户第一次给机器人发消息时，Stella 会自动把飞书频道身份链接到这个登录用户，无需执行 `/link`。
...
1. 用户发送消息。
2. 自动注册只会在机器人实际处理这条消息时触发。在群聊里，默认 `group_mode` 是 `mention`，所以用户必须先 `@` 机器人；除非你把群组覆盖配置改成 `always`。
3. Stella 确定机器人的租户键：
...
自动注册是 best-effort 的：如果租户探测失败，或联系人 API 查询失败，消息仍然按正常通道流程继续处理，但不会创建 Stella 用户。如果已经存在匹配的飞书 OAuth 登录身份，Stella 会从该登录身份自动建立频道身份链接，而不是创建一个独立用户。
...
2. 在 `group_mode: mention` 的群里，用户必须 @提及机器人才能触发自动注册。
...
只有在你希望群里每条消息都触发自动注册和回复时，才使用 `group_mode: "always"`。否则保持 `mention`，并确保用户第一次联系时先 @提及机器人。
