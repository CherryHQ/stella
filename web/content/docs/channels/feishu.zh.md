...
为某个飞书频道实例开启自动注册后，Stella 会在该机器人的租户员工第一次发消息时，自动为其创建 Stella 账号，无需手动注册或执行 `/link`。
...
自动注册是 best-effort 的：如果租户探测失败，或联系人 API 查询失败，消息仍然按正常通道流程继续处理，但不会创建 Stella 用户。
...
每个飞书用户都会通过平台身份自动识别。Stella 优先使用 `union_id`，对于旧链接回退到 `open_id`。这使得跨多个飞书应用的多实例配置能正常工作。
