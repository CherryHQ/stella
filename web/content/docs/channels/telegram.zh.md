...
- `mention` -- 被 @提及时一定会路由给被提及的机器人；其他明确问题在存在可用路由模型时，也可能由 Stella 的语义群聊路由选中回复，否则保持静默。这是默认模式。
...
| `group_mode`  | 群组行为：`mention`、`always`、`disabled`。@提及是确定性触发；无 @提及消息可能由语义路由处理。 | `mention` |
