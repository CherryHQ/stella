...
Stella 开始处理你的消息时会给它加上 🤔，回复送达后清除该表情。只有失败的回合会留下标记 👎。私聊、群组和论坛话题都适用。

成功时刻意不加表情：回复本身就是信号，而 Telegram 每次设置表情都会在你正盯着的消息上播放一次动画。此外 Telegram 只允许使用固定清单内的表情，所以失败用的是 👎，而不是 Discord 渠道用的 ❌。表情只是提示：即使机器人没有添加表情的权限，回复本身也不受影响。
...
> **注意：** 在 **管理 -> 默认模型** 配置普通会话的图片基线。**Input** 声明了 `image` 的模型只在当前回合收到像素；没有已存基线的群组历史使用不可用标记。
...
群消息默认必须 @机器人。命令也遵守同一规则：开启必须 @机器人时，在群里使用 `/help@your_bot`，或回复机器人消息。关闭**必须 @机器人**后可以使用 Stella 的群聊协作；同时还要在 BotFather 中关闭 bot privacy mode，让机器人能读取普通消息。
