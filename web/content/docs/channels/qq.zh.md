...
每个 QQ 用户会从其平台身份自动识别。会话按用户和 agent 分别管理。无需手动设置用户。渠道绑定的 agent 优先；未绑定时，私聊使用用户的默认 agent，并在未配置时回退到第一个启用的 agent。每条群消息会唤醒所有合格成员 agent，再由每个成员的本地确定性 triage 决定是否发言。
