...
关闭**允许服务器频道**时，所有服务器消息都会被忽略。默认情况下，未 @提及机器人的服务器消息不会作为独立消息轮次处理，也不会调用 agent。任何能访问机器人可读频道的成员都可以 @机器人，因此请使用 Discord 频道和 Role 权限控制访问，并只把机器人邀请到可信服务器。Agent 输出不会触发 `@everyone` 等 Discord 提及。
