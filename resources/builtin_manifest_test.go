...
func TestGenerateBuiltinManifestNestedRootAndExecutable(t *testing.T) {
	...
	manifest, err := GenerateBuiltinManifest(root, nil)
	if err != nil {
		t.Fatalf("GenerateBuiltinManifest: %v", err)
	}
	if len(manifest.Skills) != 1 || manifest.Skills[0].Root != "core/demo" {
		t.Fatalf("skills = %#v, want core/demo root", manifest.Skills)
	}
...
func TestGenerateBuiltinManifestDerivesAndValidatesPluginOwner(t *testing.T) {
	root := t.TempDir()
	writeTestBuiltin(t, root, "plugins/tool/demo/demo/SKILL.md", "---\nname: demo\ndescription: Demo\n---\nbody\n", 0o644)
	known := map[string]struct{}{"tool/demo": {}}
	manifest, err := GenerateBuiltinManifest(root, known)
	if err != nil {
		t.Fatalf("GenerateBuiltinManifest: %v", err)
	}
	...
	if _, err := GenerateBuiltinManifest(root, map[string]struct{}{"tool/other": {}}); err == nil || !strings.Contains(err.Error(), "unknown plugin owner") {
		t.Fatalf("unknown owner error = %v, want unknown owner", err)
	}
}

func TestGenerateBuiltinManifestRejectsMalformedOwnershipLayout(t *testing.T) {
	root := t.TempDir()
	writeTestBuiltin(t, root, "system/demo/SKILL.md", "---\nname: demo\n---\nbody\n", 0o644)
	if _, err := GenerateBuiltinManifest(root, map[string]struct{}{}); err == nil || !strings.Contains(err.Error(), "core/<skill>") {
		t.Fatalf("malformed layout error = %v, want strict layout rejection", err)
	}
}
...
func TestGenerateBuiltinManifestRejectsInvalidSource(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			root := t.TempDir()
			test.setup(t, root)
			_, err := GenerateBuiltinManifest(root, nil)
...
func TestBuiltinManifestGenerationIsDeterministic(t *testing.T) {
	root := t.TempDir()
	writeTestBuiltin(t, root, "core/demo/SKILL.md", "---\nname: demo\n---\n", 0o644)
	writeTestBuiltin(t, root, "core/other/SKILL.md", "---\nname: other\n---\n", 0o644)
	first, err := GenerateBuiltinManifest(root, nil)
	if err != nil {
		t.Fatal(err)
	}
	second, err := GenerateBuiltinManifest(root, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func testBuiltinRegistry(t *testing.T) *Registry {
	...
	manifest, err := GenerateBuiltinManifest(filepath.Join(source, "skills"), nil)
	if err != nil {
		t.Fatal(err)
	}
