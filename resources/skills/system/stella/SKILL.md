...
description: >
  ...
  memory system (LCM), file knowledge base, scheduled jobs, reusable workflows, goals (objectives that converge through acceptance), workers/decomposition/dependencies,
...
6. **Remembered-fact retrieval** — active `subject=world` facts are searched on demand with `memory.search_knowledge`; these are not callable skills
7. **File knowledge retrieval** — eligible private user-and-agent sessions can search the four-scope file knowledge base on demand with `knowledge_search`
...
- **Four memory spaces**: Constraints (hard user-approved rules), Identity (agent soul + user profile), Conversation (messages/summaries), and remembered facts (`subject=world`). Facts are long-term memory; skills are reusable procedures; constraints are explicit manual rules.
- **Per-user memory**: Each user has dedicated memory per agent stored in the database. User profile, soul, and constraints are injected into your system prompt for the session snapshot; active remembered facts are search-first and must be retrieved with `memory.search_knowledge` when relevant. Session tools may read profile/soul/history, but durable writes happen through Reflect for user profile or through manual memory settings. Recommended profile structure: `## User Preferences`, `## About the User`, `## Notes`. Keep it high-level — like how a person remembers someone they know. User preferences can customize your behavior but never override your core identity or rules.
...
- **Remembered facts**: Remembered facts are stored as `subject=world` (v1 `scope=user_agent`) and are not injected into the prompt by default. Use the memory tool action `search_knowledge` with a compact fact-oriented query to retrieve snapshot-visible facts. Skills do not store fact/context knowledge and must not use `metadata.knowledge_type`. Background Structured Reflect may generate and reconcile durable `subject=world` facts; normal session tools must not write facts or use skills as a substitute write path.
- **File knowledge base**: Uploaded PDF, DOCX, Markdown, and plain-text files are parsed into searchable chunks across `system`, `system_agent`, `user`, and `user_agent` scopes. In eligible private sessions, use `knowledge_search` for information likely to live in company or personal files, then cite the returned filename and page or heading locator. This tool is distinct from `memory.search_knowledge`: it searches managed files rather than remembered facts. V1 management is available on the current agent's Knowledge tab and in Settings; file preview/download, replacement, external-source sync, and OCR are not supported.
