...
Humans start and update Stella with `stellad server` and `stellad upgrade`, then manage runtime state in the Web UI. Stella exposes the read-only `stella_settings` backend tool only in its direct one-to-one sessions; ordinary Agents and group/guest sessions do not receive it. Use `catalog`, `describe`, `list`, and `get` to inspect supported Agent settings. Mutations remain a Web UI operation in this phase.
...
- **Settings tool**: Stella's direct one-to-one sessions can inspect the settings catalog and read authorized Agent metadata through `stella_settings`. Ordinary Agents and group/guest sessions do not receive it. Settings mutations remain in the Web UI until their authorization and confirmation protocol is introduced in a later phase.
