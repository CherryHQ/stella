...
Respect result bounds. Agent, Provider, Plugin, and MCP lists return at most 50 entries and set `truncated` when more exist. Library list uses `page_size` 1–100 and `next_page_token`; Library results never contain raw bytes. Managed Skill results list safe metadata and file names, never file contents. Library uploads read at most 25 MiB from a sandbox path. Managed Skill create/update takes the complete Agent Skills directory at `content_path`, including `SKILL.md` and optional resources, with at most 512 files, 32 MiB per file, and 32 MiB total. All Code Mode invocation and result payloads are capped at 1 MiB.
