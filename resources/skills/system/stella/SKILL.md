...
- **CLI authorization**: `gh` continues to use Stella's GitHub OAuth connection and receives a refreshed runtime token. `lark-cli` is independent of Stella OAuth: Stella bootstraps the current Agent's enabled Feishu/Lark Channel app into each private user workspace, then lark-cli's native device flow owns that employee's scopes, token storage, and refresh. Do not use the `oauth` tool or `/auth feishu` to repair lark-cli authorization.
