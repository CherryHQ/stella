...
Explicit destructive group and Agent deletion are Home-purging lifecycles. User deletion has the same internal lifecycle primitive, pending product account-delete integration. Purge workers use durable, exclusive, expiring claims. Cooperating LocalStore participants that share and preserve the configured lock namespace also use a cross-process advisory operating-system lock; it is not tamper-resistant against Stella's operating-system identity or privileged host operators, while isolating providers prevent Agents from reaching it. A `ready` registry row never recreates a missing root, and missing, non-directory, or symlink roots fail resolution. A retained pin detects replacement only during its bounded revalidation interval; there is no durable inode identity against trusted host-side replacement across operations or restart. Run restore, migration, and root cleanup stopped or with consumers fenced. Routine upgrades and Helm uninstall do not purge Homes. If a physical purge is retained as `purge_failed`, use `stellad storage retry-purge --help` for retry syntax.
