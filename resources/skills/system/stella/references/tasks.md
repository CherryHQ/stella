...
All of these are `stella task ...` (or `stella task goal ...`) commands; run `--help` for the exact flags. `stella task create` and `stella task goal create` need an agent; inside an agent session they default to `STELLA_AGENT_ID`, and outside Stella you should pass `--agent-id` explicitly.
