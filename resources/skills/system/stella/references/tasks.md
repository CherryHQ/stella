...
Decomposition is automatic: `stella goal create` produces a composite whose planning runs on its own, materializing children and activating them so the dispatcher runs them — no manual steps. When a goal needs a human approval gate (`review_policy=human`), the manual flow takes over: `stella goal plan <id>` proposes its `{children, edges}` (a draft revision), `stella goal approve <id> <rev>` accepts and materializes them, `stella goal activate <id>` flips the composite and its children to ready, and `submit-review`/`reject` cover the review gate. See each subcommand's `--help` for the JSON shape and flags.
