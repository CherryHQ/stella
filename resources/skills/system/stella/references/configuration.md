...
1. Call a directly listed hot tool for a standalone operation or when the model needs its result before deciding the next step. Keep standalone or potentially long-running shell, file, git, package, script, and process work as direct `bash`; use `view_image` directly.
2. Use `code` for a tool that is not directly listed, or for a short chain whose intermediate results should stay between tools. Invoke `bash` inside Code only when its output feeds another tool in that same script. The complete chain must fit Code's 30-second wall-clock budget.
3. Treat directly listed names and names documented by a loaded skill as exact. Search when the capability or tool name is unknown; when an exact name is known but its input schema is not, describe that name directly. `tools.search(query, offset?)` returns up to 20 summaries; when a non-empty search has at most three total matches, each result includes `inputSchema` and can be invoked immediately. Describe a selected search result only when it omitted its schema. An empty query lists the catalog and pages expose `hasMore` / `nextOffset`; do not enumerate it as routine discovery.
...
The model never receives a complete mutable Skill authority root. After identity and actor/Agent policy authorization, `skills.load` copies only the selected exact current revision to the active sandbox Session's temporary directory and returns that disposable execution path. Historical, deleted, disabled, deprecated, and other actors' revisions are not part of that view.
