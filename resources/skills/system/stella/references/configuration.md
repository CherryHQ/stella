...
An upgrade with existing PostgreSQL `skill_file` bytes queues their migration in the background and starts serving immediately. Managed Skill writes serialize behind the migration lock; reads may be temporarily unavailable. Stella inventories the source twice, quarantines conflicting flat filesystem mirrors, publishes and verifies immutable Home revisions, then atomically records completion and scrubs the legacy bytes. Invalid legacy source data disables managed Skills and is reported without blocking unrelated server capabilities.
