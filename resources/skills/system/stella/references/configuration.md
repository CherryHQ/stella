...
3. Treat directly listed names and names documented by a loaded skill as exact. Search only when the capability or tool name is unknown. `tools.search(query, offset?)` returns up to 20 summaries; when a non-empty search has at most three total matches, each result includes `inputSchema` and can be invoked immediately. Use `tools.describe(name)` only when the selected search result omitted its schema. An empty query lists the catalog and pages expose `hasMore` / `nextOffset`; do not enumerate it as routine discovery.
