...
> **前置条件：** 先阅读 [`./lark-shared.md`](./lark-shared.md) 了解认证、身份切换和安全规则。
...
用 `web` skill 的 `web.ts fetch` 获取顶层索引，找到与需求相关的模块文档链接：

```
bun $SKILL/scripts/web.ts fetch https://open.feishu.cn/llms.txt
...
用 `web` skill 的 `web.ts fetch` 获取模块文档，找到具体 API 的文档链接：

```
bun $SKILL/scripts/web.ts fetch https://open.feishu.cn/llms-docs/zh-CN/llms-<module>.txt
  → 提取问题："找出与 <用户需求> 相关的 API 说明和文档链接"
```

### Step 4：获取 API 完整规范

用 `web` skill 的 `web.ts fetch` 获取具体 API 文档，提取完整的调用规范：

```
bun $SKILL/scripts/web.ts fetch https://open.feishu.cn/document/server-docs/.../<api>.md
...
- [lark-shared](./lark-shared.md) — 认证和全局参数
- [lark-skill-maker](./lark-skill-maker.md) — 如需将挖掘到的 API 固化为新 Skill
