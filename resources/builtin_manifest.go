...
type BuiltinSkillDescriptor struct {
	...
	Digest     string             `json:"digest"`
	// OwnerPluginID is a required trusted release declaration, never skill frontmatter.
...
func validateBuiltinManifest(manifest BuiltinManifest) error {
	...
	for _, skill := range manifest.Skills {
		...
		if err := validateExplicitSkillOwner(skill.Root, skill.OwnerPluginID); err != nil {
			return err
		}
