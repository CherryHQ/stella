...
type BuiltinSkillDescriptor struct {
	...
	// core skills and is never read from skill frontmatter.
...
// GenerateBuiltinManifest builds the release catalog from resources/skills.
// knownOwners validates plugin directories against the shipped plugin catalog.
func GenerateBuiltinManifest(sourceRoot string, knownOwners map[string]struct{}) (BuiltinManifest, error) {
	...
	if err := discoverBuiltinSkills(sourceRoot, ".", knownOwners, &manifest.Skills, &totalFiles, &totalBytes); err != nil {
...
func discoverBuiltinSkills(sourceRoot, relative string, knownOwners map[string]struct{}, skills *[]BuiltinSkillDescriptor, totalFiles *int, totalBytes *int64) error {
	...
	if hasSkill {
		owner, err := skillOwner(relative, knownOwners)
		if err != nil {
			return err
		}
	...
	for _, entry := range entries {
		...
		if err := discoverBuiltinSkills(sourceRoot, child, knownOwners, skills, totalFiles, totalBytes); err != nil {
			return err
		}
	}
	return nil
...
func skillOwner(root string, knownOwners map[string]struct{}) (string, error) {
	...
	if knownOwners != nil {
		if _, ok := knownOwners[owner]; !ok {
			return "", fmt.Errorf("builtin skill %q has unknown plugin owner %q", root, owner)
		}
	}
	return owner, nil
}
...
func validateSkillOwner(root, owner string) error {
	derived, err := skillOwner(root, nil)
	if err != nil {
		return err
	}
	if derived != owner {
...
// WriteBuiltinManifest writes the validated catalog only when its bytes differ.
func WriteBuiltinManifest(sourceRoot, output string, knownOwners map[string]struct{}) error {
	manifest, err := GenerateBuiltinManifest(sourceRoot, knownOwners)
	if err != nil {
		return err
	}
