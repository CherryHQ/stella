...
在底层，Stella 是一个多租户、多用户、多 agent 系统，但它带给你的价值很简单：很多人可以同时依赖同一个 agent，每个 agent 都有自己的角色、模型、技能、工具、定时任务、工作区和安全边界，而控制权始终在你手里。你可以把它部署在自己选择的环境里，使用自己的模型 API 密钥，并从 Telegram、Discord、QQ、飞书、微信、Web UI 或终端访问它。
...
- **就在你已经用的聊天里。** Telegram、Discord、QQ、飞书、微信、Web UI 和终端，都是同一套 agent 的入口。
...
| 渠道    | Telegram、Discord、QQ、飞书、微信 | [渠道](/docs/channels/telegram)              |
