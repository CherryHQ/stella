...
func TestSelfRegisteredDiscordPluginIsComplete(t *testing.T) {
	...
	if !host.HasRuntime(PluginID) || !host.HasConfig(PluginID) || !host.HasStatus(PluginID) {
		t.Fatal("expected discord runtime/config/status registrations")
	}
	...
	for _, meta := range metas {
		...
		found = true
		if meta.Kind != "channel" || meta.Name != pkgchannel.PlatformDiscord || !meta.Managed || !meta.AdminVisible {
			t.Fatalf("unexpected discord metadata: %#v", meta)
		}
	}
	if !found {
		t.Fatal("expected discord metadata")
	}
}

func TestSelfRegisteredDiscordPluginAppliesAndReportsStatus(t *testing.T) {
	store := newStubStore()
	store.plugins[PluginID] = config.Plugin{ID: PluginID, Kind: "channel", Name: pkgchannel.PlatformDiscord, Enabled: true, Config: map[string]any{"token": "dc-token", "enable_notify": true}}
	...
	if got := dispatcher.Channels(); len(got) != 1 || got[0] != pkgchannel.PlatformDiscord {
...
func (*testChannel) Name() string                                          { return pkgchannel.PlatformDiscord }
