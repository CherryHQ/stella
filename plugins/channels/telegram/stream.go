...
func (b *Bot) streamEvents(c tele.Context, stream *channel.ChatStream) (string, *channel.ToolTracker, []channel.ImageEvent, error) {
	if !isGroup(c) {
		text, tracker, images, err := b.streamDraft(c, stream)
		return text, tracker, images, err
	}
	return b.streamEditEvents(c, stream, "", nil, nil)
}
...
func (b *Bot) streamDraft(c tele.Context, stream *channel.ChatStream) (text string, tracker *channel.ToolTracker, images []channel.ImageEvent, err error) {
...
func (b *Bot) streamEditEvents(c tele.Context, stream *channel.ChatStream, initial string, existing *channel.ToolTracker, existingImages []channel.ImageEvent) (string, *channel.ToolTracker, []channel.ImageEvent, error) {
	var sb strings.Builder
