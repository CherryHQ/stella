...
name: stella
description: >
  Self-knowledge for stella, the self-hosted AI assistant. Use when users ask about
  configuration, setup, onboarding, providers, models, agents, channels, webhooks,
  memory, schedules, workflows, goals, workers, skills, plugins, session compaction,
  notifications, self-update, multi-user behavior, or how stella works. Also use for
  changing models, setting up channels or webhooks, configuring providers, updating
  stella, onboarding, switching agents, reporting bugs, and goal status or acceptance.
---
...
- **Session discovery and communication**: In a one-to-one session, use `session_list` to list this agent's recent, active, or archived Sessions for the current user and `session_get` to inspect metadata, context statistics, or page through a bounded transcript. Content search belongs to `memory_search`, not `session_list`. Use `session_create` to start a focused Session and `session_send` to continue any sendable Session, including legacy delegate Sessions. Both calls are synchronous and require `wait=true`. Busy targets wait in FIFO order, with at most 32 pending inputs and a 30-second admission hold limit. Agent-originated input keeps its source-Session label and is treated as information, not human authority. Accepted input is linked to one execution; after a crash, recovery follows its terminal result without replaying the model or tools. An unknown result is not permission to repeat side effects. Use a goal when execution and acceptance must survive restarts. The tool never widens access across users or agents and is unavailable in group turns.
...
- **Managed helper CLIs**: The `bash` tool prepends Stella-managed binaries to `PATH`, and nested non-interactive Bash login shells restore that managed order after system profiles reset it. The release provides `mise` and `xberg` on supported platforms; other helper CLIs such as `fd`, `rg`, `bun`, `uv`, `gh`, `lark-cli`, and `lightpanda` are prepared from their selected Agent packages and may be unavailable on unsupported platforms.
...
- **Integration boundaries**: Providers (`anthropic`, `openai`, `openai-response`) are compiled adapters selected through the provider control plane, and sandbox backends are compiled behind an injected registry. Neither is a plugin row or managed through the plugin admin surface. Channels (`telegram`, `discord`, `qq`, `feishu`, `dingtalk`, `weixin`) still use the managed plugin host and keep their existing `channel/...` rows and `/settings/channels` Web UI. Standard Agent packages own skill, CLI, OAuth, and MCP declarations; Native agent tools, hooks, memory implementations, core sandbox tools (`bash` and `view_image`), and the scheduler-builtin Structured Reflect pipeline remain with their internal owners. Manage provider configuration and managed plugins through their respective Web UI control-plane surfaces.
