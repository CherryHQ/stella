...
func wrapCommand(policy sandboxpkg.Policy, _ string, _ []tmpMount, mounts []sessionfs.Mount, realRoot, stellaHomeHost string, name string, args []string) (execPath string, execArgs []string, err error) {
	if !seatbeltFunctional() {
	...
	profile := buildSeatbeltProfile(policy, mounts, realRoot, stellaHomeHost)
