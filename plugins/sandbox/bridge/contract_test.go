...
func TestBridgeContract(t *testing.T) {
	...
	res, err := session.Exec(ctx, `pwd; echo "$STELLA_TEST_MARK"; echo err >&2; exit 3`, sandboxpkg.ExecOptions{})
	if err != nil {
		t.Fatalf("exec: %v", err)
	}
	...
	res, _ = session.Exec(ctx, `stat -c %a "/app/子目录/数据.bin"`, sandboxpkg.ExecOptions{})
	...
	res, _ = session.Exec(ctx, `/app/skills/demo/scripts/run.sh`, sandboxpkg.ExecOptions{})
