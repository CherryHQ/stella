...
type ownershipCheckKey struct{}

func withOwnershipCheck(ctx context.Context, check func(context.Context) error) context.Context {
	if check == nil {
		return ctx
	}
	return context.WithValue(ctx, ownershipCheckKey{}, check)
}

func checkContextOwnership(ctx context.Context) error {
	check, _ := ctx.Value(ownershipCheckKey{}).(func(context.Context) error)
	if check == nil {
		return nil
	}
	return check(ctx)
...
func dialIMAP(ctx context.Context, acct EmailAccount) (*imapclient.Client, error) {
	...
	if err := checkContextOwnership(ctx); err != nil {
		return nil, err
	}
