-- The recursive Deliverable. goal = root (parent_id NULL); children are
-- deliverables, same shape all the way down. Completion is DERIVED: agents never
-- write acceptance_state — it is a cached projection over agent_dlv_acceptance_event,
...
CREATE TABLE agent_dlv_deliverable (
    ...
    -- Recursion shape.
    parent_id            TEXT REFERENCES agent_dlv_deliverable(id) ON DELETE CASCADE,  -- NULL = root (goal)
    root_id              TEXT NOT NULL REFERENCES agent_dlv_deliverable(id) ON DELETE CASCADE, -- denormalized tree root; = id for roots
    ...
    active_attempt_id    TEXT REFERENCES agent_dlv_attempt(id) ON DELETE SET NULL,
    ...
    accepted_revision_id TEXT REFERENCES agent_dlv_revision(id) ON DELETE SET NULL,
...
CREATE INDEX idx_agent_dlv_dispatchable
    ON agent_dlv_deliverable(priority DESC, created_at ASC)
...
CREATE INDEX idx_agent_dlv_rollup_candidates
    ON agent_dlv_deliverable(root_id, updated_at ASC)
