...
-- judgment verdict, stored as evidence. agent_dlv_deliverable.acceptance_state is a
...
CREATE TABLE agent_dlv_acceptance_event (
    ...
    attempt_id          TEXT REFERENCES agent_dlv_attempt(id) ON DELETE SET NULL, -- the attempt whose output was evaluated
    seq                 INTEGER NOT NULL,                                       -- monotonic per deliverable; the projection folds in seq order
    ...
    reviewer_attempt_id TEXT REFERENCES agent_dlv_attempt(id) ON DELETE SET NULL, -- the purpose='review' run, when authority='agent'
...
-- Idempotent fold + dedup: one result per (deliverable, attempt, item, cache_key).
CREATE UNIQUE INDEX uniq_agent_dlv_accept_evt
    ON agent_dlv_acceptance_event(deliverable_id, attempt_id, item_id, cache_key);
-- Check-result cache probe: hit by cache_key on the latest pass.
CREATE INDEX idx_agent_dlv_accept_evt_cache
    ON agent_dlv_acceptance_event(cache_key, created_at DESC)
