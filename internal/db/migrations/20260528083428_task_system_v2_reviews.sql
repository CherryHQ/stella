...
CREATE TABLE `new_agent_task` (
  ...
  CONSTRAINT `3` FOREIGN KEY (`agent_id`) REFERENCES `settings_agent` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
  CONSTRAINT `4` FOREIGN KEY (`user_id`) REFERENCES `auth_user` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE,
  CONSTRAINT `5` FOREIGN KEY (`org_id`) REFERENCES `auth_organization` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE,
...
INSERT INTO `new_agent_task` (`id`, `org_id`, `user_id`, `agent_id`, `title`, `description`, `status`, `priority`, `required`, `retry_count`, `max_retries`, `not_before`, `deadline_at`, `session_id`, `active_run_id`, `active_blocker_id`, `context`, `output`, `created_at`, `updated_at`, `completed_at`, `cancelled_at`) SELECT `id`, `org_id`, `user_id`, `agent_id`, `title`, `description`, `status`, `priority`, `required`, `retry_count`, `max_retries`, `not_before`, `deadline_at`, `session_id`, `active_run_id`, `active_blocker_id`, `context`, `output`, `created_at`, `updated_at`, `completed_at`, `cancelled_at` FROM `agent_task`;
...
CREATE TABLE `new_agent_task_event` (
  ...
  CONSTRAINT `3` FOREIGN KEY (`task_id`) REFERENCES `agent_task` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE,
  CHECK (actor_type IN ('system','user','agent','worker','reviewer'))
);
-- Copy rows from old table "agent_task_event" to new temporary table "new_agent_task_event"
INSERT INTO `new_agent_task_event` (`id`, `task_id`, `run_id`, `blocker_id`, `event_type`, `from_status`, `to_status`, `actor_type`, `actor_id`, `detail`, `created_at`) SELECT `id`, `task_id`, `run_id`, `blocker_id`, `event_type`, `from_status`, `to_status`, `actor_type`, `actor_id`, `detail`, `created_at` FROM `agent_task_event`;
...
-- Create "new_agent_task_blocker" table
CREATE TABLE `new_agent_task_blocker` (
  `id` text NOT NULL,
...
-- Copy rows from old table "agent_task_blocker" to new temporary table "new_agent_task_blocker"
INSERT INTO `new_agent_task_blocker` (`id`, `task_id`, `kind`, `status`, `question`, `detail`, `resolution`, `created_by_run_id`, `created_at`, `resolved_at`) SELECT `id`, `task_id`, `kind`, `status`, `question`, `detail`, `resolution`, `created_by_run_id`, `created_at`, `resolved_at` FROM `agent_task_blocker`;
-- Drop "agent_task_blocker" table after copying rows
DROP TABLE `agent_task_blocker`;
-- Rename temporary table "new_agent_task_blocker" to "agent_task_blocker"
ALTER TABLE `new_agent_task_blocker` RENAME TO `agent_task_blocker`;
