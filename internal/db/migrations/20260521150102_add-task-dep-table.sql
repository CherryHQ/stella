...
-- Extract existing JSON deps into a temp table before the old agent_task is dropped
CREATE TEMPORARY TABLE `_temp_deps` AS
  SELECT t.id AS task_id, d.value AS dep_id, t.created_at
  FROM `agent_task` t, json_each(t.deps) d
  WHERE t.deps != '[]' AND t.deps != '';
...
-- Migrate extracted JSON deps into the new edge table
INSERT INTO `agent_task_dep` (`task_id`, `dep_id`, `created_at`)
  SELECT `task_id`, `dep_id`, `created_at` FROM `_temp_deps`;
DROP TABLE IF EXISTS `_temp_deps`;
