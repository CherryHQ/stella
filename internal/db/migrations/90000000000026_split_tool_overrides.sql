...
CREATE TEMP TABLE split_tool_map (
    old_name   TEXT    NOT NULL,
    new_name   TEXT    NOT NULL,
    -- introduced marks a name this migration creates. The eight recally names
    -- that only appear under the pre-#1171 "recally" union already existed
    -- before this migration, so `down` must fold them without deleting them.
    introduced BOOLEAN NOT NULL
) ON COMMIT DROP;

INSERT INTO split_tool_map (old_name, new_name, introduced) VALUES
    ...
    -- Rows written against it are still out there, so fan them out here.
...
-- Fold the action rows back onto the old name, deny-wins against whatever the
-- old row still says. A capability disabled on any action stays disabled on the
-- union, because the union would otherwise hand it back.
--
-- The pre-#1171 "recally" union is excluded: `up` only honoured rows that
-- already existed and kept them, so folding into it would mint a row this
-- migration never created, and a later re-run of `up` would fan that row's
