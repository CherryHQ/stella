// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.29.0
// source: user_agent_memory.sql

package sqlc

import (
	"context"
)

const deleteUserAgentMemory = `-- name: DeleteUserAgentMemory :exec
DELETE FROM user_agent_memory WHERE user_id = ? AND agent_id = ?
`

type DeleteUserAgentMemoryParams struct {
	UserID  int64  `json:"user_id"`
	AgentID string `json:"agent_id"`
}

func (q *Queries) DeleteUserAgentMemory(ctx context.Context, arg DeleteUserAgentMemoryParams) error {
	_, err := q.db.ExecContext(ctx, deleteUserAgentMemory, arg.UserID, arg.AgentID)
	return err
}

const getUserAgentMemory = `-- name: GetUserAgentMemory :one
SELECT user_id, agent_id, content, updated_at FROM user_agent_memory WHERE user_id = ? AND agent_id = ?
`

type GetUserAgentMemoryParams struct {
	UserID  int64  `json:"user_id"`
	AgentID string `json:"agent_id"`
}

func (q *Queries) GetUserAgentMemory(ctx context.Context, arg GetUserAgentMemoryParams) (UserAgentMemory, error) {
	row := q.db.QueryRowContext(ctx, getUserAgentMemory, arg.UserID, arg.AgentID)
	var i UserAgentMemory
	err := row.Scan(
		&i.UserID,
		&i.AgentID,
		&i.Content,
		&i.UpdatedAt,
	)
	return i, err
}

const listUserAgentMemories = `-- name: ListUserAgentMemories :many
SELECT user_id, agent_id, content, updated_at FROM user_agent_memory WHERE user_id = ? ORDER BY agent_id
`

func (q *Queries) ListUserAgentMemories(ctx context.Context, userID int64) ([]UserAgentMemory, error) {
	rows, err := q.db.QueryContext(ctx, listUserAgentMemories, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []UserAgentMemory{}
	for rows.Next() {
		var i UserAgentMemory
		if err := rows.Scan(
			&i.UserID,
			&i.AgentID,
			&i.Content,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const upsertUserAgentMemory = `-- name: UpsertUserAgentMemory :exec
INSERT INTO user_agent_memory (user_id, agent_id, content, updated_at)
VALUES (?, ?, ?, datetime('now'))
ON CONFLICT(user_id, agent_id) DO UPDATE SET
    content = excluded.content,
    updated_at = datetime('now')
`

type UpsertUserAgentMemoryParams struct {
	UserID  int64  `json:"user_id"`
	AgentID string `json:"agent_id"`
	Content string `json:"content"`
}

func (q *Queries) UpsertUserAgentMemory(ctx context.Context, arg UpsertUserAgentMemoryParams) error {
	_, err := q.db.ExecContext(ctx, upsertUserAgentMemory, arg.UserID, arg.AgentID, arg.Content)
	return err
}
