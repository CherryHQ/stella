...
func TestUserTokenStore(t *testing.T) {
	...
	if token.ID == 0 || !token.AutoGenerated || token.ExpiresAt == nil {
	...
	if byHash.ID != token.ID {
		t.Fatalf("GetUserTokenByHash ID = %d, want %d", byHash.ID, token.ID)
	}
	...
	if active.ID != token.ID {
		t.Fatalf("GetActiveUserTokenByHash ID = %d, want %d", active.ID, token.ID)
	}
	...
	if autoToken.ID != token.ID {
		t.Fatalf("GetActiveAutoUserToken ID = %d, want %d", autoToken.ID, token.ID)
	}
	...
	if _, err := db.ExecContext(ctx, `
		...
		VALUES (?, 'expired', 'hash-expired', 'anna_exp', ?)
