...
-- name: GetAttempt :one
SELECT * FROM agent_dlv_attempt WHERE id = ?;

-- name: ListAttemptByDeliverable :many
SELECT * FROM agent_dlv_attempt
WHERE deliverable_id = sqlc.arg(deliverable_id)
  AND (sqlc.narg(purpose) IS NULL OR purpose = sqlc.narg(purpose))
...
UPDATE agent_dlv_attempt
SET gaps = sqlc.arg(gaps), updated_at = datetime('now')
WHERE id = sqlc.arg(id);

-- name: SetAttemptExecutor :exec
UPDATE agent_dlv_attempt
SET executor_agent_id = sqlc.arg(executor_agent_id), updated_at = datetime('now')
WHERE id = sqlc.arg(id);
