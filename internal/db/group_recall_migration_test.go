...
const (
	groupRecallBeforeMigration = sequentialAnchor + 18
	groupRecallMigration       = sequentialAnchor + 19
)

func TestGroupRecallMigrationIsReversibleAndLeavesCurrentSchemaReady(t *testing.T) {
	db := newTestDB(t)
	...
	if _, err := provider.UpTo(ctx, groupRecallMigration); err != nil {
		t.Fatalf("reapply group-recall migration: %v", err)
	}
	assertGroupRecallCurrentSchema(t, ctx, db)
}
