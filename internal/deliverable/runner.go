...
// SessionResolver returns the live sandbox session a deliverable's deterministic
...
// this deliverable" — the runner surfaces that as ErrNoSandbox rather than
// panicking on a nil Exec.
type SessionResolver func(ctx context.Context, deliverableID string) (sandbox.Session, error)
...
var ErrNoSandbox = errors.New("deliverable: no sandbox session for deterministic check")

// sandboxCheckRunner is the production CheckRunner (contract §4.1): it executes
// a deterministic item's command in the deliverable's sandbox, derives Pass from
...
// On a miss it runs the command in the deliverable's session, derives
...
func (r *sandboxCheckRunner) Run(ctx context.Context, item AcceptanceItem, env CheckEnv) (CheckResult, error) {
	...
	sess, err := r.resolve(ctx, env.DeliverableID)
	if err != nil {
...
// broken work).
func (r *sandboxCheckRunner) probeCache(ctx context.Context, key string) (sqlc.AgentDlvAcceptanceEvent, bool) {
	if key == "" || r.q == nil {
		return sqlc.AgentDlvAcceptanceEvent{}, false
	}
