...
// constructed deliverable bundle with a controllable executor. It is the single
// seam every deliverable package test binds to; helpers here own the FK-correct
...
func newHarness(t *testing.T) *harness {
	t.Helper()
	db, err := appdb.OpenDB(filepath.Join(t.TempDir(), "dlv.db"))
	if err != nil {
	...
	h.bundle = &Service{Queries: q, Deliverable: h.svc}
	return h
}

// sessionMinter seeds a real ctx_conversation row so a deliverable created
...
// createRoot mints a root deliverable in 'draft' with the given kind and
// contract. The zero-value contract is the trivial auto-accept; pass a populated
// one for gated acceptance.
func (h *harness) createRoot(kind string, contract AcceptanceContract) sqlc.AgentDlvDeliverable {
	h.t.Helper()
	d, err := h.svc.CreateRoot(context.Background(), CreateInput{
		UserID:   h.userID,
		AgentID:  h.agentID,
		Title:    "root",
		Intent:   "test deliverable",
		Kind:     kind,
		Required: true,
		Contract: contract,
	})
...
// get loads a deliverable, failing the test on a missing row.
func (h *harness) get(id string) sqlc.AgentDlvDeliverable {
	h.t.Helper()
	d, err := getDeliverable(context.Background(), h.q, id)
	if err != nil {
