...
func edg_twoSiblings(h *harness, withEdge bool) (up, down sqlc.AgentDlvDeliverable, composite sqlc.AgentDlvDeliverable) {
	h.t.Helper()
...
func edg_readyLeaf() sqlc.AgentDlvDeliverable {
	return sqlc.AgentDlvDeliverable{Lifecycle: LifecycleReady, Kind: KindLeaf}
}
...
func TestEdgReadinessNonReadyLifecycles(t *testing.T) {
	...
	for _, c := range cases {
		r := Compute(sqlc.AgentDlvDeliverable{Lifecycle: c.lifecycle, Kind: c.kind}, nil, now)
		if r.State != c.want {
...
func TestEdgAddEdgePersistsKindAndOnFailure(t *testing.T) {
	...
	if e.DeliverableID != down.ID || e.UpstreamID != up.ID {
		t.Fatalf("edge endpoints: down=%q up=%q want %q/%q", e.DeliverableID, e.UpstreamID, down.ID, up.ID)
	}
...
func TestEdgAddEdgeCycleRejected(t *testing.T) {
	...
	upEdges, err := h.q.ListEdgeByDeliverable(ctx, up.ID)
	if err != nil {
		t.Fatalf("ListEdgeByDeliverable: %v", err)
	}
	if len(upEdges) != 0 {
...
func TestEdgWaiveEdgeClearsDepBlock(t *testing.T) {
	...
	edge, err := h.q.GetEdge(ctx, sqlc.GetEdgeParams{DeliverableID: down.ID, UpstreamID: up.ID})
	if err != nil {
