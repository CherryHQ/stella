...
// comments document.
//
// Lifecycle note (contract §2, deliverable-model.md lines 136/186): the ONLY
...
// (deliverable-model.md line 186) says the plan gate is "ready → active requires
...
func cmp_decompose(t *testing.T, h *harness, compositeID string, content DecompositionContent) sqlc.AgentDlvRevision {
	t.Helper()
...
func cmp_children(t *testing.T, h *harness, parentID string) []sqlc.AgentDlvDeliverable {
	t.Helper()
	kids, err := h.q.ListDeliverableChildren(context.Background(), nullStr(parentID))
	if err != nil {
		t.Fatalf("ListDeliverableChildren: %v", err)
	}
	return kids
}
...
func TestCompositeRollup_DecomposeMaterializeChildren(t *testing.T) {
	...
	up := childID(rev.ID, "a")
	if _, err := h.q.GetEdge(context.Background(), sqlc.GetEdgeParams{DeliverableID: down, UpstreamID: up}); err != nil {
...
func TestCompositeRollup_RequiredAcceptedCounterAndAccept(t *testing.T) {
	...
	kids := cmp_children(t, h, root.ID)
	byKey := map[string]sqlc.AgentDlvDeliverable{}
	for _, c := range kids {
