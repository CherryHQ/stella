...
// lifecycle_leaf_test.go exercises the leaf deliverable lifecycle through the
...
// clears the active pointer — it leaves the deliverable 'active' and defers the
...
func lcl_detLeaf(h *harness, maxAttempts int) sqlc.AgentDlvDeliverable {
	h.t.Helper()
...
func TestLcl_HumanVerdictLeaf(t *testing.T) {
	...
	// Stale-verdict guard (§4.2 verdictValid) on a FRESH deliverable: a PASS verdict
	// whose scope_hash does not equal the evaluated attempt's output hash is stale —
	// the fold drops it and the leaf stays blocked. (A separate deliverable because a
...
// (deliverable, attempt, item, cache_key), so a re-submitted identical verdict
...
func TestLcl_VerdictReSubmitIsIdempotent(t *testing.T) {
	...
	events, err := h.q.ListAcceptanceEventByDeliverable(context.Background(), d.ID)
	if err != nil {
