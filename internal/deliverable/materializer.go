...
// childID is the deterministic child deliverable id derived from a revision +
...
//   - for each ProposedChild: insert (skip-if-exists) a child deliverable
...
//   - for each ProposedEdge: insert agent_dlv_edge (resolve keys→ids); PK
//     collision = no-op.
...
func (s *DeliverableService) Materialize(ctx context.Context, qtx *sqlc.Queries, rev sqlc.AgentDlvRevision, parent sqlc.AgentDlvDeliverable, childSessions map[string]string) error {
	...
	root, err := qtx.GetDeliverable(ctx, parent.RootID)
	if err == nil {
	...
	for i, ch := range content.Children {
		...
		if _, err := qtx.GetDeliverable(ctx, cid); err == nil {
			continue
		...
		if _, err := qtx.CreateDeliverable(ctx, sqlc.CreateDeliverableParams{
			ID:                 cid,
	...
	for _, e := range content.Edges {
		...
		if _, err := qtx.GetEdge(ctx, sqlc.GetEdgeParams{DeliverableID: down, UpstreamID: up}); err == nil {
	...
	if err := qtx.SetDeliverableAcceptedRevision(ctx, sqlc.SetDeliverableAcceptedRevisionParams{
		AcceptedRevisionID: sql.NullString{String: rev.ID, Valid: true},
		ID:                 parent.ID,
	}); err != nil {
		return fmt.Errorf("set accepted revision: %w", err)
	}
	if err := qtx.SetDeliverableRequiredTotal(ctx, sqlc.SetDeliverableRequiredTotalParams{
