...
func TestLoadHomeSkillEmitsOnlyProviderVisibleCatalogDirectories(t *testing.T) {
	base, userID, agentID := newTestSkillStore(t)
	ctx := ctxWithUser(userID, agentID)
	view := isolatingView()
	cases := []struct {
		scope, directory, want string
	}{
		{"system", ".stella-revisions/system/digest", "/opt/stella/db-skills/.stella-revisions/system/digest"},
		{"system_agent", ".stella-revisions/agent/digest", "/opt/stella/agent-skills/.stella-revisions/agent/digest"},
		{"user", ".stella-revisions/user/digest", "/user/.agents/skills/.stella-revisions/user/digest"},
		{"user_agent", ".stella-revisions/user-agent/digest", "/workspace/.agents/skills/.stella-revisions/user-agent/digest"},
	}
	for _, tc := range cases {
		t.Run(tc.scope, func(t *testing.T) {
			store := &homeSkillFileStore{testSkillStore: base, loaded: &pkgplugins.HomeSkillFile{
				Skill:   pkgplugins.Skill{ID: "home-" + tc.scope, Scope: tc.scope, UserID: userID, AgentID: agentID, Name: "home-skill", ContentDigest: "pinned-digest"},
				Content: "Home content", Directory: tc.directory,
			}}
			out, err := NewTool(store, "", "").WithSkillDirView(view).WithReadAuthorizer(allowAllSkillReads{}).load(ctx, map[string]any{"name": "home-skill"})
			if err != nil {
				t.Fatal(err)
			}
			if !strings.Contains(out, "<skill_dir>"+tc.want+"</skill_dir>") || strings.Contains(out, "/srv/.stella/") {
				t.Fatalf("Home %s output leaked or omitted provider path: %s", tc.scope, out)
			}
		})
	}
}

func TestLoadHomeSkillRejectsMalformedDirectoryAndTouchesPinnedDigestBeforeExposure(t *testing.T) {
	base, userID, agentID := newTestSkillStore(t)
	ctx := ctxWithUser(userID, agentID)
	...
	for _, directory := range []string{"../host/private", "/host/private", `C:\host\private`} {
		t.Run(directory, func(t *testing.T) {
			store := &homeSkillFileStore{testSkillStore: base, touchErr: errors.New("usage claim failed"), loaded: &pkgplugins.HomeSkillFile{
				Skill:   pkgplugins.Skill{ID: "home-reflect", Scope: "user_agent", UserID: userID, AgentID: agentID, Name: "home-skill", Metadata: metadata, ContentDigest: "pinned-digest"},
				Content: "must not be exposed", Directory: directory,
			}}
			out, err := NewTool(store, "", "").WithSkillDirView(isolatingView()).WithReadAuthorizer(allowAllSkillReads{}).load(ctx, map[string]any{"name": "home-skill"})
			if err == nil || out != "" {
				t.Fatalf("failed runtime touch exposed output: %q, %v", out, err)
			}
			if len(store.touched) != 1 || store.touched[0] != "pinned-digest" {
				t.Fatalf("runtime touch digests = %q, want pinned digest", store.touched)
			}
		})
	}

	store := &homeSkillFileStore{testSkillStore: base, loaded: &pkgplugins.HomeSkillFile{
		Skill: pkgplugins.Skill{ID: "home-manual", Scope: "user", UserID: userID, Name: "home-skill"}, Content: "safe", Directory: "../host/private",
	}}
	out, err := NewTool(store, "", "").WithSkillDirView(isolatingView()).WithReadAuthorizer(allowAllSkillReads{}).load(ctx, map[string]any{"name": "home-skill"})
	if err != nil || strings.Contains(out, "<skill_dir>") || strings.Contains(out, "host/private") {
		t.Fatalf("malformed directory leaked into output: %q, %v", out, err)
	}
}

func TestLoadSuppressedHomeWinnerDoesNotExposeBuiltinContent(t *testing.T) {
	base, userID, agentID := newTestSkillStore(t)
	store := &homeSkillFileStore{testSkillStore: base, loaded: &pkgplugins.HomeSkillFile{Suppressed: true}}
	out, err := NewTool(store, t.TempDir(), "").load(ctxWithUser(userID, agentID), map[string]any{"name": "stella"})
	if err == nil || out != "" {
		t.Fatalf("suppressed Home winner exposed lower content: %q, %v", out, err)
	}
}
...
func seedSkillUsage(t *testing.T, store *testSkillStore, skillID string, userID string, agentID string, useCount int64, lastUsedAt time.Time) {
	...
	if _, err := store.db.Exec(context.Background(), `
		...
		VALUES ($1, $2, $3, $4, $5, $5)
		ON CONFLICT (skill_id) DO UPDATE SET use_count = excluded.use_count, last_used_at = excluded.last_used_at
