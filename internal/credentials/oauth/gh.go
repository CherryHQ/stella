...
func (b *GitHubBroker) StartDeviceFlow(ctx context.Context, userID int64) (FlowStatus, error) {
	...
	if err != nil {
		return FlowStatus{}, fmt.Errorf("oauth/gh: device auth: %w", err)
	}
...
func (b *GitHubBroker) Poll(ctx context.Context, flowID string) (FlowStatus, error) {
	...
	if !ok {
		return FlowStatus{}, fmt.Errorf("oauth/gh: unknown flow %q", flowID)
	}
...
func (b *GitHubBroker) Complete(ctx context.Context, vs VaultStore, userID int64, flowID string) error {
	...
	b.mu.Unlock()
	if !ok {
		return fmt.Errorf("oauth/gh: no authorized token for flow %q", flowID)
	}
	...
	if err != nil {
		return fmt.Errorf("oauth/gh: flow %q failed: %w", flowID, err)
	}
	if tok == nil {
		return fmt.Errorf("oauth/gh: flow %q is not yet authorized", flowID)
	}
