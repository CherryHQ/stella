...
func (m *TokenManager) GetGHToken(ctx context.Context, userID int64) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("oauth: get gh token: %w", err)
	}
	if bundle == nil {
		return "", fmt.Errorf("oauth: get gh token: user %d has not connected GitHub", userID)
	}
	if bundle.AccessToken == "" {
		return "", fmt.Errorf("oauth: get gh token: empty access token in vault for user %d", userID)
	}
...
func (m *TokenManager) GetLarkRuntimeEnv(ctx context.Context, userID int64) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("oauth: get lark env: %w", err)
	}
	if bundle == nil {
		return nil, fmt.Errorf("oauth: get lark env: user %d has not connected Lark/Feishu", userID)
	}
	...
	if now.After(bundle.RefreshExpiresAt.Add(-tokenExpirySafetyMargin)) {
		return nil, fmt.Errorf(
			"oauth: get lark env: user %d Lark refresh token expired at %s; please re-authorize",
	...
	if now.After(bundle.AccessExpiresAt.Add(-tokenExpirySafetyMargin)) {
		...
		if err != nil {
			return nil, fmt.Errorf("oauth: get lark env: refresh: %w", err)
		}
		if err := SaveLarkBundle(ctx, m.vs, userID, *refreshed); err != nil {
			return nil, fmt.Errorf("oauth: get lark env: save refreshed bundle: %w", err)
		}
		bundle = refreshed
	}
