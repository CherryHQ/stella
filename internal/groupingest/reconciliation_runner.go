...
func validateGroupReconciliationPlan(
	...
	for i := range plan.Operations {
		...
		if ref := temporaryGroupSubjectRef(op.NewContent, unit, bundle); ref != "" {
			return GroupReconciliationPlan{}, fmt.Errorf("operation %d new_content contains temporary subject_ref %q", i, ref)
		}
	}
...
func temporaryGroupSubjectRef(content string, unit GroupReviewUnit, bundle GroupRelatedBundle) string {
	...
	for _, fact := range bundle.Facts {
		if fact.SubjectRef != "" && strings.Contains(content, fact.SubjectRef) {
			return fact.SubjectRef
		}
	}
	return ""
}
