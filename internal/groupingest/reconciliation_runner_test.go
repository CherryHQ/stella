...
func TestGroupReconciliationRejectsTemporarySubjectRefInNewContent(t *testing.T) {
	unit, candidates, facts := reconciliationFixture()
	bundle, err := BuildGroupRelatedBundle(unit, candidates, facts)
	if err != nil {
		t.Fatalf("build bundle: %v", err)
	...
	plan := GroupReconciliationPlan{Operations: []GroupReconciliationOperation{{
		Operation:     memory.GroupFactActionCreate,
		CandidateRefs: []reflectpkg.CandidateRef{"group-fact-0001"},
		NewContent:    "subject-0001 coordinates all production releases.",
		Rationale:     "Invalid temporary reference.",
	}}}
	if _, err := validateGroupReconciliationPlan(plan, unit, bundle); err == nil {
		t.Fatal("temporary subject_ref in persisted content should fail")
	}
}

func TestGroupRelatedBundleRejectsNonReflectFact(t *testing.T) {
	unit, candidates, facts := reconciliationFixture()
	facts[0].Source = "manual"
	if _, err := BuildGroupRelatedBundle(unit, candidates, facts); err == nil {
		t.Fatal("non-Reflect fact should not enter the related bundle")
	}
}
