...
func TestStructuredGroupReflectDrainsMultipleWindowsUntilCaughtUp(t *testing.T) {
	db, q := openTestDB(t)
	store := eventlog.NewStore(db)
	appendHuman(t, store, "g-backlog", "alice", "one")
	last := appendHuman(t, store, "g-backlog", "bob", "two")
	groupID := resolveGroup(t, store, "test", "g-backlog", "")
	stream, calls := scriptedGroupStream(t,
		scriptedGroupResponse{
			toolName: groupGenerationTool,
			raw:      `{"candidates":[],"no_candidate_reason":"No durable collaboration fact."}`,
