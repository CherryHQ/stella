...
type countingPartsQuerier struct {
	calls int
	ids   []string
	parts []sqlc.GetMessagePartsByMessagesRow
}

func (q *countingPartsQuerier) GetMessagePartsByMessages(_ context.Context, ids []string) ([]sqlc.GetMessagePartsByMessagesRow, error) {
	q.calls++
...
func TestLoadMessagePartsUsesOneBatchQuery(t *testing.T) {
	q := &countingPartsQuerier{parts: []sqlc.GetMessagePartsByMessagesRow{
		{CtxMessagePart: sqlc.CtxMessagePart{ID: "p1", MessageID: "m1", PartType: "text", Ordinal: 0, TextContent: pgtype.Text{String: "first", Valid: true}}},
		{CtxMessagePart: sqlc.CtxMessagePart{ID: "p2", MessageID: "m2", PartType: "image", Ordinal: 0, MediaID: pgtype.Text{String: "media-2", Valid: true}}},
	}}
