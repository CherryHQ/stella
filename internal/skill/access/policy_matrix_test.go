...
func TestEmbeddedPostgresSkillAccessMatrix(t *testing.T) {
	...
	userSkillID := mustCreateSkill(t, skillStore, skill.Skill{Scope: ScopeUser, UserID: owner.ID, Name: "user-skill"})
	userAgentSkillID := mustCreateSkill(t, skillStore, skill.Skill{Scope: ScopeUserAgent, UserID: owner.ID, AgentID: "sys", Name: "user-agent-skill"})
	systemSkillID := mustCreateSkill(t, skillStore, skill.Skill{Scope: ScopeSystem, Name: "system-skill"})
	systemAgentSkillID := mustCreateSkill(t, skillStore, skill.Skill{Scope: ScopeSystemAgent, AgentID: "sys", Name: "system-agent-skill"})
	...
	// user scopes.
	userSkill := skill.Skill{ID: userSkillID, Scope: ScopeUser, UserID: owner.ID}
	userAgentSkill := skill.Skill{ID: userAgentSkillID, Scope: ScopeUserAgent, UserID: owner.ID, AgentID: "sys"}
	systemSkill := skill.Skill{ID: systemSkillID, Scope: ScopeSystem}
...
type skillIdentityFixture map[string]skill.Skill

func (store skillIdentityFixture) GetIdentity(_ context.Context, id string) (*skill.Skill, error) {
	skill, ok := store[id]
	if !ok {
		return nil, nil
	}
...
func mustCreateSkill(t *testing.T, store skillIdentityFixture, sk skill.Skill) string {
	t.Helper()
	if sk.Status == "" {
		sk.Status = "active"
	}
