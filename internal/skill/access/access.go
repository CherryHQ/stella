...
func (s *Service) ManageByID(ctx context.Context, authority authz.Authority, id string, action authz.Action) (skill.Skill, error) {
	access, err := s.Begin(ctx, authority)
	if err != nil {
...
func (a *Access) AuthorizeManage(ctx context.Context, sk skill.Skill, action authz.Action) error {
	if a.userID == "" {
		return ErrForbidden
	}
	// A delegated agent never touches another agent's bound skill.
...
func (a *Access) AuthorizeRead(ctx context.Context, sk skill.Skill) error {
	...
	// agent gate. A delegated agent never reads another agent's bound skill.
	if a.scopeAgentID != "" && sk.AgentID != "" && a.scopeAgentID != sk.AgentID {
		return ErrNotFound
	}
...
func (a *Access) AuthorizeManageByID(ctx context.Context, id string, action authz.Action) (skill.Skill, error) {
...
func (a *Access) load(ctx context.Context, id string) (skill.Skill, error) {
	row, err := a.svc.store.GetIdentity(ctx, id)
	if err != nil {
		return skill.Skill{}, fmt.Errorf("%w: get skill identity: %w", ErrUnavailable, err)
	}
	if row == nil {
		return skill.Skill{}, ErrNotFound
	}
	return *row, nil
}
