...
func (p *XbergParser) Available(ctx context.Context) error {
	if p == nil {
	...
	if err := p.runner.Probe(probeCtx, p.config.Binary, &stdout, &stderr); err != nil {
...
func (p *XbergParser) decode(data []byte) ([]ParsedChunk, error) {
	...
	for index, chunk := range envelope.Result.Chunks {
		...
		if !hasEffectiveText(content) {
			return nil, fmt.Errorf("%w: chunk %d has no effective text", ErrInvalidXbergJSON, index)
		}
...
// truncatingBuffer bounds diagnostics without making stderr volume itself a
// parse failure. It reports successful writes so the child process continues,
// while retaining only the configured prefix for sanitized logging.
