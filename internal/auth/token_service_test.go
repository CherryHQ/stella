...
func (s *fakeTokenStore) CreateUserToken(_ context.Context, token UserToken) (UserToken, error) {
	for _, existing := range s.tokens {
		if existing.AutoGenerated && existing.UserID == token.UserID && existing.AgentID == token.AgentID && existing.RevokedAt == nil {
...
func (s *fakeTokenStore) GetActiveAutoUserToken(_ context.Context, userID int64) (UserToken, error) {
	for _, token := range s.tokens {
		if token.UserID == userID && token.AutoGenerated && token.AgentID == "" && token.RevokedAt == nil {
			return token, nil
		}
	}
	return UserToken{}, sql.ErrNoRows
...
func (s *fakeTokenStore) GetActiveAutoAgentToken(_ context.Context, userID int64, agentID string) (UserToken, error) {
	for _, token := range s.tokens {
		if token.UserID == userID && token.AgentID == agentID && token.AutoGenerated && token.RevokedAt == nil {
			return token, nil
		}
	}
	return UserToken{}, sql.ErrNoRows
...
func (s *fakeTokenStore) RevokeAutoAgentToken(_ context.Context, userID int64, agentID string) (int64, error) {
	now := time.Now().UTC()
	var count int64
