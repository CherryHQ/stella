...
func TestEngine_BuiltinPolicies_UserSystemAgents(t *testing.T) {
	engine := NewEngineFromPolicies(builtinPolicies)
	ctx := context.Background()
	...
	if !engine.Can(ctx, AccessRequest{
		...
		t.Error("user should be able to read system agents")
	}
}

func TestEngine_BuiltinPolicies_UserAssignedAgents(t *testing.T) {
	engine := NewEngineFromPolicies(builtinPolicies)
	ctx := context.Background()
	...
	if !engine.Can(ctx, AccessRequest{
		...
		Resource: Resource{Type: ResourceAgent, ID: "agent-x"},
	}) {
		t.Error("user should be able to execute assigned agent")
	}
	...
	if engine.Can(ctx, AccessRequest{
		...
		Resource: Resource{
			Type:  ResourceAgent,
			ID:    "agent-z",
			Attrs: map[string]any{"scope": "private"},
		},
	}) {
		t.Error("user should not be able to execute unassigned non-system agent")
	}
}

func TestEngine_BuiltinPolicies_UserViewAgentsList(t *testing.T) {
	engine := NewEngineFromPolicies(builtinPolicies)
	ctx := context.Background()
	...
	if !engine.Can(ctx, AccessRequest{
		...
		t.Error("user should be able to view agents list")
	}
}
