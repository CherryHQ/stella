...
var RecallyRSSTemplate = JobTemplate{
   ...
   - rss: call the native recally_feed_poll tool with limit=20 once. Omit id to poll every enabled RSS feed; non-rss feeds are skipped server-side.
   - twitter / website (and other non-rss kinds): call the native recally_feed_list tool, and for each feed whose kind matches that workflow, follow the recally skill (e.g. the Twitter or website workflow) to list items and push them via recally_entry_add (dedup on guid).
