...
var RecallyRSSTemplate = JobTemplate{
   ...
   - rss: call the native recally tool with action=feed_poll and limit=20 once. Omit id to poll every enabled RSS feed; non-rss feeds are skipped server-side.
   - twitter / website (and other non-rss kinds): call the native recally tool with action=feed_list, and for each feed whose kind matches that workflow, follow the recally skill (e.g. the Twitter or website workflow) to list items and push them via action=entry_add (dedup on guid).
