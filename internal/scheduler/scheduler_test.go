...
func TestAddWorkflowJobValidation(t *testing.T) {
	...
	if _, err := svc.addJobInternal(t.Context(), addJobSpec{Name: "wf", Message: "message", Schedule: Schedule{Every: "1h"}, SessionMode: "reuse", UserID: "user-1", OwnerKind: JobOwnerUser, ExecScope: ExecScopeUser, DispatchKind: DispatchKindWorkflow, Payload: map[string]any{"workflow_id": "wf-1"}, Enabled: true}); !errors.Is(err, ErrWorkflowJobValidation) {
