...
func TestScopedVaultPermissionsAndRuntimeResolution(t *testing.T) {
	...
	if _, err := q.CreateAgent(ctx, sqlc.CreateAgentParams{
		ID: "restricted-a", Name: "Restricted A", Model: "test/model", Workspace: "workspace", Sandbox: "{}", EnabledBuiltinSkills: "[]", Scope: "restricted", Enabled: true,
	}); err != nil {
...
func TestScopedTokenVaultAgentBinding(t *testing.T) {
	...
	for _, id := range []string{"sa-agent-a", "sa-agent-b"} {
		if _, err := q.CreateAgent(ctx, sqlc.CreateAgentParams{
			ID: id, Name: id, Model: "test/model", Workspace: "workspace", Sandbox: "{}", EnabledBuiltinSkills: "[]", Scope: "system", Enabled: true,
		}); err != nil {
...
func TestSystemVaultMutationsInvalidateRunners(t *testing.T) {
	...
	if _, err := sqlc.New(env.db).CreateAgent(context.Background(), sqlc.CreateAgentParams{
		ID: "sys-agent", Name: "Sys", Model: "test/model", Workspace: "workspace", Sandbox: "{}", EnabledBuiltinSkills: "[]", Scope: "system", Enabled: true,
	}); err != nil {
		t.Fatalf("CreateAgent: %v", err)
	}
