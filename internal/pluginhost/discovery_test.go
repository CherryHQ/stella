...
func TestAdminVisibleDiscoveryAlwaysShowsOneSandboxActive(t *testing.T) {
	store := &stubStore{plugins: map[string]config.Plugin{
		"sandbox/docker": {ID: "sandbox/docker", Kind: config.PluginKindSandbox, Name: config.SandboxBackendDocker, Enabled: false},
		"sandbox/local":  {ID: "sandbox/local", Kind: config.PluginKindSandbox, Name: config.SandboxBackendLocal, Enabled: false},
		"sandbox/none":   {ID: "sandbox/none", Kind: config.PluginKindSandbox, Name: config.SandboxBackendNone, Enabled: false},
	}}
	host := New(store)
	registerSandboxMetadata(host)
	...
	for _, plugin := range plugins {
		if plugin.Kind == config.PluginKindSandbox && plugin.State.Enabled {
			active = plugin.Info.ID
		}
	}
	if active != "sandbox/local" {
		t.Fatalf("expected local sandbox to be active fallback, got %q in %#v", active, plugins)
	}
}

func TestSetEnabledKeepsOneSandboxActive(t *testing.T) {
	store := &stubStore{plugins: map[string]config.Plugin{
		"sandbox/docker": {ID: "sandbox/docker", Kind: config.PluginKindSandbox, Name: config.SandboxBackendDocker, Enabled: false},
		"sandbox/local":  {ID: "sandbox/local", Kind: config.PluginKindSandbox, Name: config.SandboxBackendLocal, Enabled: true},
		"sandbox/none":   {ID: "sandbox/none", Kind: config.PluginKindSandbox, Name: config.SandboxBackendNone, Enabled: false},
	}}
	svc := configService{store: store}

	if err := svc.SetEnabled(context.Background(), "sandbox/local", false); err != nil {
		t.Fatal(err)
	}
	if !store.plugins["sandbox/local"].Enabled {
		t.Fatal("local sandbox should stay enabled when it is the last active backend")
	}

	if err := svc.SetEnabled(context.Background(), "sandbox/docker", true); err != nil {
		t.Fatal(err)
	}
	if !store.plugins["sandbox/docker"].Enabled || store.plugins["sandbox/local"].Enabled {
		t.Fatalf("expected docker only active, got %#v", store.plugins)
	}

	if err := svc.SetEnabled(context.Background(), "sandbox/docker", false); err != nil {
		t.Fatal(err)
	}
	if store.plugins["sandbox/docker"].Enabled || !store.plugins["sandbox/local"].Enabled {
		t.Fatalf("expected local fallback after disabling docker, got %#v", store.plugins)
	}
}

func registerSandboxMetadata(host *Host) {
	for _, backend := range []string{config.SandboxBackendDocker, config.SandboxBackendLocal, config.SandboxBackendNone} {
		id := config.PluginID(config.PluginKindSandbox, backend)
		host.RegisterPluginID(id)
		host.SetInfo(pkgplugins.PluginInfo{
			ID:           id,
