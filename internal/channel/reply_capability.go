...
// NewDurableReplyCapabilityResolver constructs the trusted composition-root
// resolver. A nil return means the deployment cannot resolve encrypted reply
// capabilities and publishers requiring one must fail closed.
...
func (r *durableReplyCapabilityResolver) Resolve(ctx context.Context, id, channelID, kind string) (DurableReplyCapability, error) {
	if r == nil || r.q == nil || r.vault == nil {
		return DurableReplyCapability{}, fmt.Errorf("reply capability resolver unavailable")
	}
