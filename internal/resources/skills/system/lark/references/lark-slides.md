...
**CRITICAL — 生成任何 XML 之前，MUST 先用 Read 工具读取 [xml-schema-quick-ref.md](./lark-slides/xml-schema-quick-ref.md)，禁止凭记忆猜测 XML 结构。**
...
也可以分两步（先创建空白 PPT，再逐页添加），详见 [+create 参考文档](./lark-slides/lark-slides-create.md)。

> 以上是最小可用示例。更丰富的页面效果（渐变背景、卡片、图表、表格等），参考下方 Workflow 和 XML 模板。

## 执行前必做

> **重要**：`./lark-slides/slides_xml_schema_definition.xml` 是此 skill 唯一正确的 XML 协议来源；其他 md 仅是对它和 CLI schema 的摘要。
...
完整 bash 模板和 `+media-upload` 参数见 [+media-upload 文档](./lark-slides/lark-slides-media-upload.md)。
...
可直接复制使用的模板（封面页、内容页、数据卡片页、结尾页）：[slide-templates.md](./lark-slides/slide-templates.md)
...
| [lark-slides-xml-presentations-create.md](./lark-slides/lark-slides-xml-presentations-create.md) | 创建空白 PPT 命令详情 |
