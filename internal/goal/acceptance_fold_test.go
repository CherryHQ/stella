...
// DB-free: tests build []sqlc.AgentGoalAcceptanceEvent literals directly. The
...
func accf_detEvent(seq int64, itemID, result string) sqlc.AgentGoalAcceptanceEvent {
	return sqlc.AgentGoalAcceptanceEvent{
		Seq:       seq,
		ItemID:    itemID,
...
func accf_judgEvent(seq int64, itemID, result, scopeHash string) sqlc.AgentGoalAcceptanceEvent {
	return sqlc.AgentGoalAcceptanceEvent{
		Seq:       seq,
		ItemID:    itemID,
...
func TestAcceptanceFold_PolicyAll(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			c:         twoReq,
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultPass), accf_detEvent(2, "b", ResultPass)},
			wantState: AcceptancePassed,
		},
		{
			name:      "one required fail => failed",
			c:         twoReq,
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultPass), accf_detEvent(2, "b", ResultFail)},
			wantState: AcceptanceFailed,
		},
		{
			name:      "missing required => pending",
			c:         twoReq,
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultPass)},
...
func TestAcceptanceFold_PolicyAny(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultFail), accf_detEvent(2, "b", ResultPass)},
			wantState: AcceptancePassed,
		},
		{
			name:      "all required fail => failed",
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultFail), accf_detEvent(2, "b", ResultFail)},
			wantState: AcceptanceFailed,
		},
		{
			...
			events:    []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "a", ResultFail)},
			wantState: AcceptancePending,
		},
	}
	for _, tc := range cases {
	...
	t.Run("a pass clears pending and verdict bookkeeping", func(t *testing.T) {
		...
		events := []sqlc.AgentGoalAcceptanceEvent{accf_detEvent(1, "b", ResultPass)}
		got := DeriveAcceptance(c, accf_currentHash, events)
		if got.State != AcceptancePassed {
