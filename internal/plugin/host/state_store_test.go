...
func TestStateStoreRejectsStaleAgentRunMutation(t *testing.T) {
	db := dbtest.New(t)
	q := sqlc.New(db)
	...
	firstStore := agentrun.NewStore(db, firstBootID)
	t.Cleanup(firstStore.Close)
	if err := firstStore.RegisterBoot(t.Context()); err != nil {
		t.Fatalf("register first executor boot: %v", err)
	}
	first, err := firstStore.Acquire(t.Context(), sessionID, "web")
	if err != nil {
	...
	secondStore := agentrun.NewStore(db, secondBootID)
	t.Cleanup(secondStore.Close)
	if err := secondStore.RegisterBoot(t.Context()); err != nil {
		t.Fatalf("register replacement executor boot: %v", err)
	}
	second, err := secondStore.Acquire(t.Context(), sessionID, "web")
	if err != nil {
