...
var invocationInventory = map[string]map[string]int{
	...
	// Durable run worker's turn entry: the run row's persisted actor facts are
	// re-checked via access.Use against the exact session, then the rebuilt
	// authority drives svc.Chat — this is the access-authorized adapter for
	// durable runs, not a bypass.
	"internal/channel/run_executor.go": {"GetService": 1, "Chat": 1},
