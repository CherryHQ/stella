...
func (s *ProjectStore) Ensure(ctx context.Context, agentID, userID string) (string, error) {
	...
	userHome, err := SetupUserWorkspace(config.StellaHome(), userID, agentID)
	if err != nil {
		return "", err
	}
	...
	go func() {
		if err := HydrateUserAssets(context.Background(), config.StellaHome(), userHome); err != nil {
	...
	baseDir := UserAgentDir(config.StellaHome(), userID, agentID)
