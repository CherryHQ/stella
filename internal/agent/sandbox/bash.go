...
func (t *hostBashTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	if norm.IsError {
		...
		if result.ExitCode < 0 {
			return hooks.RedactSecretValues(norm.Content, secretValues), fmt.Errorf("bash: command was killed before it exited (code %d)", result.ExitCode)
		}
		...
		return hooks.RedactSecretValues(norm.Content, secretValues), &ai.CommandExitError{Tool: "bash", ExitCode: result.ExitCode}
	}
	return hooks.RedactSecretValues(norm.Content, secretValues), nil
}
