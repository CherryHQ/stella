...
- **Plugins**: Plugin tools (`plugins/tools/`) self-register via init() and are auto-discovered at startup. 10 built-in plugins: 1 tool (webfetch), 4 channels (telegram, qq, feishu, weixin), 2 hooks (trace, rtk), 3 providers (anthropic, openai, openai-response). Core tools (read/bash/edit/write/agent) are always enabled and are not plugins. Manage with `anna plugin list/enable/disable/config`.
