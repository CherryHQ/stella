...
- **Plugins**: Plugin tools (`plugins/tools/`) self-register via init() and are auto-discovered at startup. 11 built-in plugins: 2 tools (`mcp`, `webfetch`), 4 channels (telegram, qq, feishu, weixin), 2 hooks (trace, rtk), 3 providers (anthropic, openai, openai-response). Core tools (read/bash/edit/write/agent) are always enabled and are not plugins. The `mcp` plugin lets admins configure multiple MCP servers/transports in the admin UI, starts them in the background when enabled, and exposes one generic `mcp` tool. When using MCP tools, inspect prompt-listed MCP tool IDs and always call `mcp` with `action="get"` before `action="exec"`. Manage plugins with `anna plugin list/enable/disable/config`.
