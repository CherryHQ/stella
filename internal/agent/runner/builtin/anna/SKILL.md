...
- **Sandboxing**: On Linux and macOS, agents run through a `boxsh` sandbox with copy-on-write filesystem isolation. Each session gets an ephemeral overlay that doesn't modify the source workspace. Network access is disabled by default and can be set to `disabled`, `allow_all`, or `whitelist` per agent. Path traversal outside the workspace is blocked. Runner startup fails closed when the sandbox backend is unavailable.
