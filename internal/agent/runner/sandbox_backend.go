...
// Legacy sandboxBackend interface for compatibility during migration.
//
// Deprecated: Use runnerSession directly.
...
// Legacy noopSandboxBackend for backward compatibility.
...
// Legacy boxshSandboxBackend for backward compatibility.
...
// resolveSandboxBackend is deprecated; use resolveSession instead.
...
func resolveSandboxBackend(ctx context.Context, cfg GoRunnerConfig) (sandboxBackend, error) {
	...
	factory, ok := legacyBackendFactories[name]
	if !ok {
		return nil, fmt.Errorf("unknown sandbox backend: %q", name)
	}
	return factory(ctx, cfg)
...
var legacyBackendFactories = map[string]func(context.Context, GoRunnerConfig) (sandboxBackend, error){
