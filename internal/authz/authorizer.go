...
// one-interface-per-implementation ceremony. The concrete revision-verified
// implementation lands in internal/authz/policy in subphase B; this file fixes
// the contract shape that domain services and that implementation must satisfy.

// ErrAuthorizerUnavailable is the fail-closed sentinel a Begin implementation
// returns when it cannot establish a policy revision (revision read failure,
// reload failure). A caller that receives it must deny the use case.
...
// Begin once to bind itself to one immutable, revision-validated policy view;
...
// Evaluation is an immutable policy view bound to one revision. Every Decide in
// the same use case observes the same revision without another database read; a
// new use case (each privileged tool call, durable worker action, or protected
// stream-event delivery) reacquires a fresh Evaluation via Begin.
