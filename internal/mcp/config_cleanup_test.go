...
func TestCommonConfigDeleteCleansOnlyAuthorizedCredentialFamily(t *testing.T) {
	...
	if _, err := svc.pool.Exec(ctx, `UPDATE plugin_config SET scope='system',user_id=NULL,config=jsonb_set(config,'{mcp_servers,main,credential_mode}','"per_user"'),credential_refs='{}'::jsonb WHERE id=$1`, configID); err != nil {
		t.Fatal(err)
	}
