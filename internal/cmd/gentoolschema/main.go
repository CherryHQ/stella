...
const (
	...
	outputDir = "internal/tools/domaintools"
)
...
func run(input, outDir string) error {
	...
	for _, name := range sortedToolNames(tools) {
		content, err := renderTool(name, tools[name])
		if err != nil {
			return err
		}
		path := filepath.Join(outDir, name+"_gen.go")
...
func renderTool(tool string, actions []toolAction) ([]byte, error) {
	...
	exportTool := exportName(tool)
	var out bytes.Buffer
	fmt.Fprintf(&out, "// Code generated by go run ./internal/cmd/gentoolschema; DO NOT EDIT.\n\n")
	out.WriteString("package domaintools\n\n")
	out.WriteString("import (\n\t\"context\"\n\t\"fmt\"\n)\n\n")
	fmt.Fprintf(&out, "func %sInputSchema() map[string]any {\n", exportTool)
	fmt.Fprintf(&out, "\treturn mustDomainToolInputSchema(%sInputSchemaJSON)\n", exportTool)
	out.WriteString("}\n\n")
	fmt.Fprintf(&out, "const %sInputSchemaJSON = `", exportTool)
	out.Write(schemaJSON)
	out.WriteString("`\n\n")
	fmt.Fprintf(&out, "type %sHandler interface {\n", exportTool)
	for _, action := range actions {
		fmt.Fprintf(&out, "\t%s(context.Context, %s%sInput) (any, error)\n", exportName(action.Action), exportTool, exportName(action.Action))
	}
	out.WriteString("}\n\n")
	for _, action := range actions {
	...
	fmt.Fprintf(&out, "func Dispatch%s(ctx context.Context, h %sHandler, action string, args map[string]any) (any, error) {\n", exportTool, exportTool)
	out.WriteString("\tswitch action {\n")
	for _, action := range actions {
		fmt.Fprintf(&out, "\tcase %q:\n", action.Action)
		fmt.Fprintf(&out, "\t\tvar in %s%sInput\n", exportTool, exportName(action.Action))
		fmt.Fprintf(&out, "\t\tif err := decodeDomainToolInput(args, &in, %#v); err != nil {\n\t\t\treturn nil, err\n\t\t}\n", action.Required)
