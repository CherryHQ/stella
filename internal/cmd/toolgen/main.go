...
func run(input, declDir, outRoot string) error {
	...
	for _, family := range sortedKeysOf(families) {
		...
		out := filepath.Join(outDir, generatedFileName)
		expected[out] = true
...
const generatedFileName = "tool_gen.go"

// generatedMarker is the header every rendered file carries. Pruning keys off
// it rather than off the file name, so a hand-written tool_gen.go — if anyone
// ever writes one — is never deleted.
const generatedMarker = "// Code generated by go run ./internal/cmd/toolgen; DO NOT EDIT."
...
func pruneStaleOutputs(outRoot string, expected map[string]bool) error {
	return filepath.WalkDir(outRoot, func(path string, entry fs.DirEntry, err error) error {
		...
		if entry.IsDir() || entry.Name() != generatedFileName || expected[path] {
			return nil
		}
...
// inputTypeName is the generated Go input type for one tool. A declared family
// prefixes its family name — SessionSendInput, not SendInput — because it
// generates into a package that already has hand-written types of its own
// (internal/agent/session/access already declares a SendInput), and a redeclared
// type is a compile error the declaration author never asked for. An
// operation-backed family keeps its historical names; the two collisions it has
// are pinned in generatedNameFallbacks.
