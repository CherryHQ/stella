...
// WriteTx commits one Run-owned source-domain mutation only while the
// immutable AgentRun owner is still current. Contexts without a Guard use the
// same transaction path but have no execution lease to validate.
