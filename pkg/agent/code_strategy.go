...
//
// Recall is two tools now, not one union, so both memory names are hot: a model
// that can search but not read a returned ref cannot finish a recall
// (rules/agent-tools.md §8).
var HotToolNames = []string{"bash", "memory_read", "memory_search", "skill_load", "view_image"}
