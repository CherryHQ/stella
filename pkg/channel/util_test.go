...
func TestFileReceivedContentUsesXberg(t *testing.T) {
	blocks := FileReceivedContent("report.pdf", "$STELLA_ASSETS_DIR/202608/report.pdf")
...
func TestAttachmentReceivedContentInlinesImages(t *testing.T) {
	...
	blocks := AttachmentReceivedContent("photo.png", "$STELLA_ASSETS_DIR/202608/photo.png", data)
	if len(blocks) != 2 {
		t.Fatalf("expected note + image blocks, got %d: %#v", len(blocks), blocks)
	}
	note, ok := blocks[0].(ai.TextContent)
	if !ok || !strings.Contains(note.Text, "$STELLA_ASSETS_DIR/202608/photo.png") {
...
func TestAttachmentReceivedContentFallsBackToFileHint(t *testing.T) {
	blocks := AttachmentReceivedContent("report.pdf", "$STELLA_ASSETS_DIR/202608/report.pdf", []byte("%PDF-1.7 not an image"))
	got := ai.FlattenText(blocks)
...
func TestAttachmentReceivedContentOversizedImageHintsRead(t *testing.T) {
	...
	blocks := AttachmentReceivedContent("huge.png", "$STELLA_ASSETS_DIR/202608/huge.png", data)
