...
test("registry API filters transports, classifies auth, and resumes at an upstream page boundary", async ({ admin }) => {
  const first = expectStatus(await admin.get<{ servers: RegistryServer[]; next_page_token?: string }>("/api/mcp/registry/servers?q=anything&page_size=2"), 200, "registry search");
  ...
  const second = expectStatus(await admin.get<{ servers: RegistryServer[]; next_page_token?: string }>(`/api/mcp/registry/servers?page_size=2&page_token=${encodeURIComponent(first.next_page_token!)}`), 200, "registry next page");
...
test("registry detail prefers latest, unknown ids 404, and upstream failures map cleanly", async ({ admin }) => {
  const detail = expectStatus(await admin.get<RegistryServer>(`/api/mcp/registry/servers/official/${encodeURIComponent("com.stella/registry-add")}`), 200, "registry detail");
