...
test("typing", async ({ page, creds }) => { for(let rep=0;rep<reps;rep++){await open(page,creds);await loadHistory(page);const typing=await page.evaluate(()=>window.__perf?.typeInto("textarea","the quick brown fox ".repeat(6)));results[rep].typing=typing;await page.evaluate(()=>window.__perf?.clearComposer("textarea"));} const out={label,date:new Date().toISOString(),commit:"playwright",reps,runs:results};await mkdir(resolve("perf","results"),{recursive:true});await writeFile(resolve("perf","results",`${label}.json`),JSON.stringify(out,null,2)); });
