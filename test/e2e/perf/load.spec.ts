...
test("files-load",async({page,creds})=>{for(let i=0;i<Number(process.env.REPS_LOAD??3);i++){await open(page,filesSession,creds);await installMetrics(page);runs[i].filesLoad={...(await page.evaluate(()=>window.__perf?.navStats("/messages")) as any),resTotalKB:0,resLastEndMs:null,loaded:0,total:imgCount};}await mkdir(resolve("perf","results"),{recursive:true});await writeFile(resolve("perf","results",`load-${label}.json`),JSON.stringify({label,date:new Date().toISOString(),commit:"playwright",huge_turns:hugeTurns,img_count:imgCount,pdf_count:pdfCount,runs},null,2));});
