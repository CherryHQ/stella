...
// Fake is an in-process stand-in for the Anthropic Messages API. It
...
type Fake struct {
	...
	mu           sync.Mutex
	scripts      []response            // Phase 1 FIFO queue of not-yet-served responses
	modelScripts map[string][]response // stable model-id lanes for concurrent journeys
	...
	modelTrailing map[string]response
	reqs          []Request // every request received, in arrival order
...
func (g *TurnGate) Release() { close(g.ReleaseChan) }
...
// response is one scripted assistant turn. Exactly one shape is set: a
...
// Request captures the model-request fields a journey may assert. Response
...
// New starts the fake and registers cleanup that fails the test if
...
func (f *Fake) assertConsumedLocked() {
	...
	for model, scripts := range f.modelScripts {
		if len(scripts) != 0 {
			f.failf("fake anthropic: %d scripted response(s) for model %q never consumed", len(scripts), model)
		}
	}
	for action, cs := range f.controls {
		if !cs.served {
			f.failf("fake anthropic: goal_control %q stage never requested", action)
		}
	}
...
func (f *Fake) BaseURL() string { return f.server.URL }

// EnqueueText scripts the next Phase 1 turn as a plain-text assistant reply.
func (f *Fake) EnqueueText(text string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.scripts = append(f.scripts, response{text: text})
}

func (f *Fake) EnqueueTextForModel(model, text string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.modelScripts == nil {
		f.modelScripts = make(map[string][]response)
	}
	f.modelScripts[model] = append(f.modelScripts[model], response{text: text})
}
...
// strict guard.
func (f *Fake) SetTrailingTextForModel(model, text string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.modelTrailing == nil {
		f.modelTrailing = make(map[string]response)
	}
...
func (f *Fake) EnqueueTool(id, name, args string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.scripts = append(f.scripts, response{toolID: id, toolName: name, toolArgs: args})
}
...
func (f *Fake) EnqueueGoalControl(action, args string) {
	var parsed struct {
		Action string `json:"action"`
	}
	if err := json.Unmarshal([]byte(args), &parsed); err != nil {
		f.failf("fake anthropic: goal_control %q arguments are not valid JSON: %s", action, args)
	}
	...
	if parsed.Action != action {
		f.failf("fake anthropic: goal_control stage %q carries action %q", action, parsed.Action)
	}
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.controls == nil {
...
// Called under f.mu.
func (f *Fake) goalCodeCall(id string, build func() (string, error)) (response, bool) {
	args, err := build()
	if err != nil {
		f.failf("fake anthropic: build goal code call: %v", err)
		return response{}, false
	}
	return response{toolID: id, toolName: "code", toolArgs: args}, true
}
...
func (f *Fake) EnqueueError(status int, errType, message string) {
	...
	if err != nil {
		...
		f.failf("fake anthropic: marshal error body: %v", err)
	}
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *Fake) WaitForRequests(ctx context.Context, want int) []Request {
	...
	for {
		...
		case <-ticker.C:
		case <-ctx.Done():
			f.failf("fake anthropic: waited for %d request(s): %v", want, ctx.Err())
			return nil
		}
	}
}
...
func (f *Fake) handle(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != "/v1/messages" || r.Method != http.MethodPost {
		f.failf("fake anthropic: unexpected request %s %s", r.Method, r.URL.Path)
		http.Error(w, "unexpected path", http.StatusNotFound)
		return
	}

	model, messages, images, toolNames, continuation := parseMessagesRequest(f, r)
	...
	if !isFlusher {
		...
		f.failf("fake anthropic: ResponseWriter is not a Flusher; SSE cannot be streamed")
	...
	if resp.gate != nil {
		...
		case <-time.After(gateBackstop):
			f.failf("fake anthropic: gated turn never released within %s; finishing it to avoid a suite deadlock", gateBackstop)
		}
...
func (f *Fake) writeFrames(w http.ResponseWriter, flusher http.Flusher, frames []string) bool {
...
// it cannot answer.
func (f *Fake) selectResponse(model string, goal goalTurn) (response, bool) {
	...
	if f.errScript != nil {
		...
		return response{errStatus: f.errScript.status, errBody: f.errScript.body}, true
	}
	...
	if len(f.scripts) == 0 {
		f.failf("fake anthropic: unscripted request (model=%q); no response was enqueued", model)
		return response{}, false
	}
	resp := f.scripts[0]
...
func (f *Fake) selectGoalResponse(model string, goal goalTurn) (response, bool) {
	if !goal.continuation {
		return f.goalCodeCall("toolu_goal_probe", goalProbeScript)
	}
	if goal.failure != "" {
		f.failf("fake anthropic: goal code call failed (model=%q): %s", model, goal.failure)
		return response{}, false
	}
	if goal.stage != "" {
		return response{text: goalTrailingReply}, true
	}
	action := nonFailAction(goal.actions)
	cs := f.controls[action]
	if cs == nil {
		f.failf("fake anthropic: goal attempt advertised actions %v (model=%q); no stage was enqueued for %q", goal.actions, model, action)
		return response{}, false
	}
	if cs.served {
...
// correctly.
func (r response) frames(model string) []string {
	var frames []string
...
func (r response) gatedFrames(model string) (before, after []string) {
	full := response{text: r.text}.frames(model)
	second := response{text: r.text2}.textDeltaFrame()
...
func (r response) textDeltaFrame() string {
...
func parseMessagesRequest(f *Fake, r *http.Request) (model string, messages []string, images []Image, toolNames []string, continuation bool) {
	body, err := io.ReadAll(r.Body)
	if err != nil {
		f.failf("fake anthropic: read request body: %v", err)
	...
	if err := json.Unmarshal(body, &parsed); err != nil {
		f.failf("fake anthropic: request body is not valid Messages JSON: %v", err)
		return "", nil, nil, nil, false
	}
	...
	for _, message := range parsed.Messages {
		...
		if !ok {
			f.failf("fake anthropic: unsupported message content %s", message.Content)
			continue
		}
...
func messagePayload(content json.RawMessage) (string, []Image, bool) {
	var text string
...
func collectMessageBlocks(content json.RawMessage, text *strings.Builder, images *[]Image) bool {
	...
	for _, block := range blocks {
		...
		case "image":
			...
			*images = append(*images, Image{MediaType: block.Source.MediaType, Data: block.Source.Data})
		case "tool_result":
